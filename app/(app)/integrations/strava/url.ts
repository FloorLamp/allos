...
// Public app URL) when set, else derived from the request headers (behind a
...
// The OAuth callback route MUST redirect through this rather than
...
// configured and the request host is localhost (e.g. a reverse proxy that
// doesn't forward the real host). Detected up-front so we can tell the admin to
// set the Public app URL instead of handing Strava an unreachable redirect_uri.
