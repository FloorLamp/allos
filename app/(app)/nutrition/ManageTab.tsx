...
export default async function ManageTab({
  ...
  const historyFor = (s: IntakeItem): DoseHistoryEntry[] =>
    (doseHistoryByItem.get(s.id) ?? []).map((row) => {
      ...
      // (recorded_at) — with the answer saying WHICH it was. The panel IS
  ...
  // distinctly — a curated claim and a generated one must not look alike.
  const curatedSuggestions = getCuratedSupplementSuggestions(profile.id);
  ...
  const suggestionPanel = (
    <>
      {/* CURATED (#2378) — the deterministic half. Present whether or not AI is
          configured, because it never calls a model: the same flagged labs yield the
          same options every time, from a committed, human-reviewed map. */}
      {curatedSuggestions.length > 0 && (
        <div className="mb-4" data-testid="curated-supplement-panel">
          <p className="section-label mb-2">From your labs — curated</p>
          <CuratedSupplementSuggestions suggestions={curatedSuggestions} />
          <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
            These come from a committed, human-reviewed map of well-established
            biomarker&rarr;supplement pairs, screened against your allergies,
            medications and conditions. No dose is suggested — that, and whether
            to start anything at all, is a conversation with your clinician.
          </p>
        </div>
      )}
      {/* #3970 rule 1. The AI-origin disclaimer is CONSTANT across this section, so
          it states itself once on the section's own label instead of mounting an
          `h-8 w-8` button beside every Generated badge. It stays an InfoTooltipIcon
          and never a `title=` — #3375 made this #2378 disclaimer reachable on touch
          and by keyboard, and that constraint binds the single mount too. */}
