...
import { getActivityFaults, getJournalSourceKeys } from "@/lib/queries";
import {
  activityProvenanceKeyLabel,
  JOURNAL_SOURCE_MANUAL,
} from "@/lib/journal-format";
import type { DayGroup } from "@/lib/journal-card";
import type { JournalFilters } from "@/lib/journal-filters";

// ONE resolution of "what does the Journal feed show right now" (issue #1634),
...
// filter changes (loadJournalPage). Before this existed the two forked: the action
...
export interface ResolvedJournalFeed {
  groups: DayGroup[];
...
export interface JournalFeedContext extends ResolvedJournalFeed {
...
export async function resolveJournalFeed(
  filters: JournalFilters,
  before: string | null = null
): Promise<ResolvedJournalFeed> {
  return (await resolveJournalFeedContext(filters, before)).feed;
}

// The feed plus the once-per-request context the controls need. HistorySection uses
// this; the action uses resolveJournalFeed above and drops the rest.
export async function resolveJournalFeedContext(
  filters: JournalFilters,
  before: string | null = null
): Promise<{ feed: ResolvedJournalFeed } & JournalFeedContext> {
  ...
  for (const id of contextIds) {
    for (const key of getJournalSourceKeys(id)) sourceKeys.add(key);
  ...
  const sourceOptions = [...sourceKeys]
    .sort((a, b) =>
      a === JOURNAL_SOURCE_MANUAL
        ? -1
        : b === JOURNAL_SOURCE_MANUAL
          ? 1
          : a.localeCompare(b)
    )
