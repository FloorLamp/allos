...
// Serve a stored training photo (#3285 item 3). The progress-photo serve posture,
// unchanged: session-gated (the Edge middleware only checks cookie presence) and
// scoped by `id AND profile_id`, so one profile can never fetch another's race or
// session photos by id. Path-contained to the training-photo root, image-only,
...
export async function GET(
  ...
  const row = db
    .prepare(
      `SELECT stored_path, thumb_path, mime_type FROM training_photos
