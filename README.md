...
- **Benchmarks** — estimated 1-rep maxes (Epley) and strength standards relative to bodyweight. Each core barbell lift (squat, bench, overhead press, deadlift, and the weighted pull-up) also gets a **bodyweight-band strength-standard** coaching line on its exercise detail — where your estimated 1RM sits among beginner→elite standards **for your exact bodyweight and sex**, and how far to the next level ("at the intermediate standard for men at your bodyweight — 12 kg to advanced"). Thresholds are **derived, not scraped** (no proprietary tables): the project's own anchor ratios scaled by bodyweight^(2/3) — the cross-sectional-area law (Lietzke 1956) — and interpolated between bodyweight bands, baked into `lib/strength-standards.json` (regenerated with `npm run gen:strength-standards`); shown only when sex and a bodyweight are on file, informational only
