...
- **Nutrition** — a one-tap food-group serving log (habit tier by design, not a calorie counter) with a today/yesterday backfill toggle, weekly habit targets (paced on-track / on-pace / behind), and food-first suggestions from flagged labs — both **eat-more** (a low nutrient → its curated food sources) and **cut-back** (a high core-panel marker → the limit-tier foods to reduce) — optionally loggable from Telegram (Settings → Profile)
