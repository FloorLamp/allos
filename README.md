...
**Food tracking rides on Health Connect.** MyFitnessPal, Cronometer, Lose It!, and Yazio have no usable direct API, but they can all write your logged meals to Health Connect — so that's the supported way to get nutrition into Allos. Turn on Health Connect sync in your food app, connect **Google Health Connect** here, and your daily calories and protein/carbs/fat land on **Trends → Body → Macros**. Logged protein also becomes the tracked basis for the protein-adequacy card on **Nutrition**, so its estimate reflects what you actually ate instead of a bodyweight guess.
