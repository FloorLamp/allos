...
- **Supplements & medications** — split across **Nutrition → Supplements** (supplement stacks, NIH UL/RDA checks, adherence-pattern observations, AI suggestions) and a standalone **Medications** page (courses, side effects, stop/restart); both share tri-state dose confirms (taken / skipped / clear), adherence & refill tracking, drug–drug / supplement–drug / food–drug interaction warnings, a **pharmacogenomics (PGx) cross-check** (a stored PGx result flags affected meds with CPIC-cited, informational guidance — never prescriptive), and context-aware scheduling (pre/post-workout, rest-day, situational). Cross-kind interaction/PGx warnings show on both surfaces; the old `/medicine` link redirects to the Supplements tab
