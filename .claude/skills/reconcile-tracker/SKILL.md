...
- Before closing, comment with the absorbing issue, then use the granted
  `mcp__github__issue_write` operation with `not_planned` for a fold. These actions
...
within the granted tools: this skill has scoped MCP readers, confined script
writers, and the MCP issue writer, not general shell REST access.
