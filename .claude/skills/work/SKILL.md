...
`docs/work.md` and its procedure files are CANONICAL — this skill
...
- The MCP set — squash merges, draft-to-ready, protected-ref and Actions
  writes — is the worker's alone where MCP is granted; without it,
...
- The taxonomy is `KNOWN_LABELS` (`reconcile-tracker-core.ts`). Never invent
  ...
  a new label for every past mistake (`docs/work/labels.md`).
...
- Sweep the open queue about every four hours for new filings, label drift,
  and comment rulings (`docs/work/lifecycle.md` owns the cadence).
...
§Dispatch has the numbers. Only the worker runs full E2E suites.
...
any other worker's branches (`docs/work/multi-worker.md`).
...
`docs/work/review-merge.md` §Review is the checklist; the posture
that makes it work:
...
```bash
node scripts/work/dispatch-brief.mjs done <branch>
```
