...
test.describe("Settings → Server: off-volume backup status", () => {
  test("shows the off-volume copy status in the backup card", async ({
    ...
    await expect(offsite.getByText(/BACKUP_DEST_DIR/).first()).toBeVisible(); // eslint-disable-line no-restricted-properties -- first-ok: asserts the BACKUP_DEST_DIR hint renders in the scoped offsite card — order-agnostic presence
  });
});
