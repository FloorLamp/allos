...
// whole story; the affordance is gated on a configured Light tier, so this asserts
// whichever state holds (a sibling admin spec may briefly configure a tier) without
// depending on global config — the manual add path is ALWAYS present.
test("free-text intake is suggest-only and degrades to taps offline", async ({
  page,
}) => {
  await page.goto("/");
  const bar = page.getByTestId("symptom-log-bar").first();
  ...
  // suggest-only (a "Suggest" button, not a silent write).
