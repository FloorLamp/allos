...
// and back-to-empty assertions are repeat-safe and no shared profile is touched.
//
    ...
    (h) =>
      (
        h
          ...
          .get(SICK_VIDEO_PROFILE) as { id: number }
      ).id
  );
}
...
function cleanup() {
  withDb((h) => {
    const pid = (
      h
        .prepare("SELECT id FROM profiles WHERE name = ?")
        .get(SICK_VIDEO_PROFILE) as { id: number } | undefined
    )?.id;
    if (pid == null) return;
