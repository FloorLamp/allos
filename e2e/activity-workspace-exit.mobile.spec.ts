...
test("a new activity closes from the header ✕, and there is no backdrop behind it", async ({
  ...
  const close = page.getByTestId("close-activity");
  await expect(close).toBeVisible();
  await close.click();

  // Nothing to discard, so nothing to ask: the form is simply gone.
...
test("the ✕ asks before discarding an edit the form cannot save", async ({
  ...
  await followLink(
    page,
    page.getByTestId("analyze-sessions").getByRole("link").first(), // first-ok: any stored session reaches the editor under test
    /\/training\/activity\/\d+$/
  );
...
test("a live workout finishes from the sticky footer, and Save closes the workspace", async ({
  ...
  const footer = page.getByTestId("activity-form-footer");
