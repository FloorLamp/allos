...
test.describe("smoking history (issue #83)", () => {
  test("records status + pack-years + quit year and persists across reloads", async ({
    ...
    await expect(page.getByLabel("Saved").first()).toBeVisible(); // eslint-disable-line no-restricted-properties -- first-ok: asserts a Saved autosave indicator appears (several fields save) — order-agnostic
