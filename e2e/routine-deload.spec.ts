...
test("Today's session card shows deload copy in the deload week (#741)", async () => {
  ...
  await expect(card.getByText("3 × 5–8").first()).toBeVisible(); // eslint-disable-line no-restricted-properties -- first-ok: several exercises in this scoped card share the 3×5–8 deload scheme; assert it renders at all
