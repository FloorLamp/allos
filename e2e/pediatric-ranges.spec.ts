...
// /results/clinical-results/view; blood pressure is a CONTINUOUS vital and renders on its metric
...
test.describe.serial("pediatric reference ranges", () => {
  ...
  test("child ALP is judged against the age band, not the adult range", async ({
    ...
    await page.goto(
      "/results/clinical-results/view?name=Alkaline%20Phosphatase"
    );
  ...
  test("child blood pressure shows an AAP percentile + category", async ({
    ...
    await switchProfile(page, "Riley (child)");
    await page.goto(
      "/results/clinical-results/view?name=Blood%20Pressure%20Systolic"
    );
