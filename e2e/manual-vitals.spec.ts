...
// #1851: the four measures that were charted, imported and consumed downstream
// but had no field. This drives the REAL form — the field names it posts are the
...
test("the measurements form takes water, lean/bone mass, respiratory rate and a bed/wake window (#1851)", async ({
  page,
}) => {
  const form = await openMeasurementsForm(page);
  ...
  // The night's two clocks reach FormData under the names the action reads: the
