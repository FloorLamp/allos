...
export function seedIllness(): void {
  ...
  // Member logins granted the SHARED seeded profiles — profile 1 ("admin") + profile 2
  // ("Riley (child)", = rileyId) — so specs stop creating members at runtime through
  // Settings → Family (a router.refresh() render path that went stale under CI load — the
  // create-member census flake). Grant sets are STATIC; the specs never mutate them, and
  // profile 1 (lowest id) is the caregiver's default active profile.
  ...
  if (rileyId) {
    ...
    console.log("e2e: seeded illness-episode caregiver fixtures (#868)");
  }
