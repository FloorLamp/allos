...
test("the Medications row shows a 'requires monitoring' note for a monitored drug", async ({
  ...
  ).toHaveAttribute("href", "/results/clinical-results?q=INR");
  const addResult = detailNote.getByRole("link", { name: "Add INR result" });
  const addResultHref = await addResult.getAttribute("href");
  expect(addResultHref).toBe(
    "/results/clinical-results?new=1&name=INR#add-result"
  );
...
test("a med-driven monitoring retest surfaces on the Upcoming page", async ({
  ...
  await expect(
    page.getByText("Monitoring labs for Warfarin", { exact: false }).first() // eslint-disable-line no-restricted-properties -- first-ok: asserts the Warfarin monitoring line renders at all — order-agnostic presence
  ).toBeVisible();
});
