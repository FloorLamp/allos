...
test("the clinical results catalog filters by a clinical panel slug (#1502)", async ({
  page,
}) => {
...
test("biomarker detail links to the rest of its panel (#1502)", async ({
  ...
  await page.goto(
    "/results/clinical-results/view?name=" +
      encodeURIComponent("LDL Cholesterol")
  );
  ...
  ).toHaveAttribute("href", "/results/clinical-results?panel=lipids");
});
...
test("an un-canonicalized reading keeps its stored heading as provenance (#1502)", async ({
  ...
  await page.goto(
    "/results/clinical-results?q=" + encodeURIComponent("E2E Novel Lab")
  );
  const table = page.getByTestId("clinical-results-table");
...
const IMPORT_ROW_URL =
  "/results/clinical-results?q=" + encodeURIComponent(IMPORT_ROW);
...
test.describe("the phone card (#2316)", () => {
  ...
  test("reaches the source document from the row's ⋯ menu", async ({
    ...
    const link = page.getByTestId("clinical-result-source-document-link");
...
test("a read-only household row keeps a ⋯ menu holding only the source document (#2316)", async ({
  ...
  try {
    ...
    await page.goto(
      "/results/clinical-results?q=" + encodeURIComponent(MVBIO_RO_ANALYTE)
    );
    const row = page
      .getByTestId("clinical-results-table")
      .locator("tr")
