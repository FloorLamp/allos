...
// a top-level row (Protocols is still adult-only, Wellness/Progress photos still
...
const TOP_LEVEL_ORDER: (string | RegExp)[] = [
  ...
  // Protocols and Progress photos. Collapsed on "/" (no child route is active),
...
// Trends took History's place here in #4965 — History earned a top-level row
// (#4918 made `/history?day=` a deliberate daily visit) and Trends lost its
// dock slot to the same 2026-08-29 review that took its permanent row (#4102).
...
// it, fails this.
const PLAN_REVIEW_CHILDREN = ["Upcoming", "Trends", "Wellness", "Protocols"];
...
test("the phone drawer renders the group inline while the desktop sidebar still folds (#3343)", async ({
  ...
  const [headerLabel, childLabel] = await settledBoxes([
    ...
    group.getByText("Trends", { exact: true }),
  ]);
  expect(
    childLabel.x,
