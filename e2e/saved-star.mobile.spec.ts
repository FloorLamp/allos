...
// Lipoprotein(a) + the weight metric) and un-stars it again through the tile's own
// control, restoring the seed state — so the spec is repeat-safe under `--repeat-each`
// and perturbs no neighbour. No exact count of a shared-seed row is asserted.
...
test("starring a biomarker gives it a Trends chart tile with no second gesture", async ({
  ...
  // …and the SAME star, in the tile footer, un-stars it (one gesture both ways).
  const tileStar = savedRow
    .getByTestId("trend-mini-card")
    .filter({ hasText: ANALYTE })
    .getByTestId("star-toggle");
  await expect(tileStar).toHaveAttribute("aria-pressed", "true");
...
test("a saved metric tile is promoted, not hidden, and carries a reorder control", async ({
  page,
}) => {
  // Kind-specific meaning: for a standard metric a save is PROMOTION — every metric
  // tile renders either way. The seed saves the weight metric, so the saved row leads
  // with it while the unsaved metric tiles still render below.
  await page.goto("/trends");
  const savedRow = page.getByTestId("saved-tiles");
  await expect(savedRow).toBeVisible();

  const savedTiles = savedRow.getByTestId("trend-mini-card");
  await expect(savedTiles.first()).toBeVisible(); // first-ok: the saved row's lead tile is the subject — "does the saved set lead the grid?"

  // The reorder affordance that replaced the pin toggle: present on saved tiles, and
  // disabled at the top of the list (an end move is a no-op, so it isn't offered).
  await expect(
    savedTiles.first().getByTestId("saved-move-up") // first-ok: same lead tile — its "move earlier" is the one that must be disabled
  ).toBeDisabled();

  // Unsaved metric tiles still render (promotion, not visibility) and carry the star
  // to save them, but no reorder control.
  const unsavedTile = page
    .getByTestId("trend-mini-card")
    .filter({ hasNotText: "Weight" })
    .last();
  await expect(unsavedTile.getByTestId("star-toggle")).toBeVisible();
});
...
test("the picker stars a biomarker straight from Trends", async ({ page }) => {
  ...
  // gesture as the ★ anywhere else. Create-and-clean: it stars an analyte the seed
  ...
  await settledClick(page, tile.getByTestId("star-toggle"));
  await expect(
