...
// documents on profile 1 plus a body-fat reading sourced from each, so Trends →
// Body's "Compare sources" body_fat card carries two DISTINCT document series. They
// used to both collapse to one "Document" label and one teal color; now each shows
// its filename and its own de-collided color, and the primary-source picker's two
// document options are distinguishable.
test.describe("Source comparison — per-document identity (#533)", () => {
  test("two documents get distinct labels and colors, not one 'Document'", async ({
    ...
    // Every legend color dot is distinct — the two documents no longer share the
    // one fallback teal.
