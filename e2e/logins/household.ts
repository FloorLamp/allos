...
// ── Unroutable reminders (issue #2173) ───────────────────────────────────────
// A spec-owned caregiver fixture for e2e/household-setup.spec.ts. Unroutable is a
// DERIVED VERDICT over a whole profile's configuration — the #2353 rule applied one
// level up from rows — so it owns profiles nothing else writes rather than asserting
// over a shared one. A neighbour adding a dose or a notification channel anywhere else
// can then never flip them.
//
//   • OK    — one dosed active supplement and a PROFILE-scoped Home Assistant
//             webhook, which is the one channel that routes with no managing login.
...
//             no channel).
//   • QUIET — never onboarded, with an all-inactive roster: nothing would send.
