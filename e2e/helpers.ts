...
// affordance instead. All of them open the SAME panel, so this takes whichever is
// there rather than making every caller work out which state it is in.
export function partOptionsOpener(page: Page, index = 0): Locator {
  const part = page.getByTestId("activity-part").nth(index); // nth-ok: the caller names which part it means; 0 is the only one in a single-part form
  return part
...
export async function openPartOptions(page: Page, index = 0): Promise<void> {
  await partOptionsOpener(page, index).click();
