...
test("a dose confirmed offline queues, then replays as a real taken dose (#1427)", async ({
  ...
  await ledger.getByRole("button", { name: "Take", exact: true }).click();
  ...
  await expect(
    reloaded.getByRole("button", { name: "Undo take", exact: true })
  ...
  await expandLedgerDueGroups(page);
  await expect(
    ledgerDoseRow(page, name).getByRole("button", {
      name: "Undo take",
      exact: true,
    })
  ).toHaveAttribute("aria-pressed", "true");
...
test("a queued confirm that lands on an already-skipped dose is surfaced, not silently synced (#1427)", async ({
  ...
  await expect(
    reloaded.getByRole("button", { name: "Take", exact: true })
  ).toHaveAttribute("aria-pressed", "false");
