...
test.describe("Immunizations (#391)", () => {
  test("the list renders the schedule + recorded doses, and a vaccine detail shows dose history", async ({
    ...
    await expect(
      page.locator('a[href^="/immunizations/"]').first() // eslint-disable-line no-restricted-properties -- first-ok: follows any immunization detail link — order-agnostic navigation
    ).toBeVisible();
    ...
    await expect(
      page.getByText("Hepatitis B Surface Antibody").first() // eslint-disable-line no-restricted-properties -- first-ok: asserts the seeded HepB titer row renders — order-agnostic presence
    ).toBeVisible();
