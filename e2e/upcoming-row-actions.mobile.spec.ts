...
//
// These assertions are about composition, not pixels:
//   1. still exactly one overflow trigger per row at phone width,
...
//
// Runs against the same spec-owned record-free adult fixture as the desktop
// spec, so its /upcoming is dense with preventive rows.
...
// documented gotcha). Load-bearing here: the whole point is the sub-`sm` layout.
const PHONE_CONTEXT = {
  viewport: { width: 390, height: 844 },
  hasTouch: true,
} as const;

const loginAsFixture = (browser: Parameters<typeof loginAs>[0]) =>
  loginAs(
    browser,
    { username: E2E_LOGIN_UPCOMING_ROWS, password: E2E_MEMBER_PASSWORD },
    PHONE_CONTEXT
  );

test.describe("Upcoming row actions at phone width (#1446)", () => {
  test("each row keeps one overflow trigger, folds its secondary chips, and fits the viewport", async ({
    browser,
  }) => {
    test.slow();

    const page = await loginAsFixture(browser);
    try {
      ...
      for (let i = 0; i < count; i++) {
        ...
        // (1) One kebab per row still holds at 390px — the census saw a card
        // with two of them STACKED here.
        ...
        // (4) The trailing control line stays a single line. An orphaned "⋯"
        // means the controls wrapped, which doubles this box's height.
      ...
      // (2)+(3) On a preventive row the secondary chips fold: "Book" is not an
      // inline control at this width, but it IS in the row's one menu. This is
      // the half that keeps the fold honest — a care-tier action may be moved
      // behind a kebab on a phone, never removed (#449).
      const bookingRow = main
        .locator('[data-testid^="upcoming-item-visit:"]')
        .first(); // first-ok: spec-owned fixture — any bookable visit row proves the fold
      await expect(bookingRow).toBeVisible();
      await expect(bookingRow.getByRole("link", { name: "Book" })).toBeHidden();

      await bookingRow.getByLabel("More actions").click();
      const menu = page.getByRole("menu");
      await expect(menu.getByRole("menuitem", { name: "Book" })).toBeVisible();
