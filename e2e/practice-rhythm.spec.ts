...
// THE PROTOCOL PRACTICE ROW RETIRED WITH THE PROTOCOL FAMILY (#5435 §4).
//
// This asserted that the dashboard's `protocol.practice:` row is titled by the noun
// the practice is called rather than by the verb that logs it — "Sauna", never
// "Log Sauna" — and that the whole row is one link with no separate control. §4 takes
// protocol rows off Home; the practice's own surfaces keep them.
//
// WHAT RETIRED WITH IT: the naming rule as asserted on `/`. What did not: the rule
// itself, which is `frequencyScopeLabel`'s and is covered in lib/__tests__; and the
// PRACTICE TARGET's Home row, which is a different family — a due action in its own
// right under the Now rule (§3.2) — covered in
// lib/__db_tests__/dashboard-placement-manifest.test.ts with both its converses.
