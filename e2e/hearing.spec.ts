...
test.describe("Hearing / audiology (#713, #717, #1600)", () => {
  ...
  test("seeded audiogram thresholds render + flag on Clinical results (#713)", async ({
    ...
    await expect(main.getByText(/Hearing Threshold/).first()).toBeVisible(); // eslint-disable-line no-restricted-properties -- first-ok: asserts the Hearing Threshold readout renders — order-agnostic presence
    await expect(main.getByText(/4 kHz/).first()).toBeVisible(); // eslint-disable-line no-restricted-properties -- first-ok: asserts a 4 kHz frequency label renders — order-agnostic presence
  });
