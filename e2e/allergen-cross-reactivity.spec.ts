...
test("Allergies page shows the birch cross-reactivity note (#153)", async ({
  ...
  const item = page.getByTestId("cross-reactivity-item").first(); // eslint-disable-line no-restricted-properties -- first-ok: the curated cross-reactivity dataset renders Birch first (deterministic static order); asserted below
