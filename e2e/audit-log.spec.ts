...
test.describe("Settings → Audit log", () => {
  test("admin sees the Audit tab and a login event row", async ({ page }) => {
    ...
    await expect(
      page.getByTestId("audit-table").getByText("login.success").first() // eslint-disable-line no-restricted-properties -- first-ok: asserts a login.success audit entry EXISTS at all — order-agnostic presence on a growing log
    ).toBeVisible();
  });
