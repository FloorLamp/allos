...
test("each set groups its identity and options above its values at 390px (#1612)", async ({
  ...
  // ONE LAYOUT, whatever the mix of plans and records (#5762): the load is stated
  // once above the rows, under its own unchanging name, and every row is reps.
  // Set / Options stay off the phone schema.
...
//
// It lives HERE rather than in a file of its own because the grid at 390px is what this
// spec is about, and a new spec FILE re-partitions all twelve CI shards — changing who
// every other spec runs beside for one test that belongs to a surface already covered.
//
// It brings its own login: FORM_PLATEAU is a dedicated profile whose Skullcrusher has
// five sessions of 30 kg × 8 × 3 — the three-set history the plan is built from — where
// the rest of this file drives the shared profile through the `page` fixture (#868).
...
test("a three-set plan finishes as the two sets that happened (#5373)", async ({
  ...
  try {
    ...
    const sheet = await openLogSheet(page);
    await (await showLogRow(sheet, "log-activity")).click();
    await pickPlannedLift(page, "Skullcrusher");
