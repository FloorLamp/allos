...
and the current host's [agent tools](../../../docs/orchestration/environment.md#agent-tools).
Check available CLI/API capabilities instead of assuming `gh` or a particular
MCP tool exists. Missing write access does not cancel reads or drafted rulings;
report only the write that cannot be performed. Respect actual permission refusals
and verify every successful write by rereading the item.
...
Use the host's question tool, or ordinary conversation if unavailable. Batch up
to four questions within the tool's actual limit, continuing until drained or the
owner stops. Asynchronous questions leave dependent work pending while workers
continue ready work under [dispatch](../../../docs/orchestration/dispatch.md).
Per question:
...
- **Mechanics:** PATCH bodies and POST comments through an authorized structured
  API argument, JSON stdin (`gh api --input -`), or a body file prepared with an
  available file-writing tool (`gh api ... -X PATCH -F body=@file.md`). Preserve
  literal text and newlines; a missing scratch-file writer does not block an
  available structured request. Re-read to verify — a ruling that
...
An authorized PATCH can update both arrays together: start from a fresh issue
read, remove only `needs-human` and the resolved owner assignment, and preserve
other labels and assignees. Do not switch verbs or transports to evade a sandbox
or approval refusal; follow the current session's permission process.
...
- **A merge gate, now satisfied** → route to the authorized orchestrator for the
  current [merge checks](../../../docs/orchestration/review-merge.md#merge), using
  an available authorized transport. Gate stated but unmet → leave
