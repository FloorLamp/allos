...
| Dated READING (the same quantity in `body_metrics` / `metric_samples` / `medical_records`, #1997)   | `readingIdentity()` (which IS `biomarkerFamily`, applied one level up) plus the stream ↔ canonical map `STREAM_READING_SOURCES`                                                                                                                      | `lib/reading-model.ts`                                                                             |
