...
| Substance (curated + custom, spelling/case variants)                                                | `normalizeSubstanceName()`, `resolveSubstanceKey()`, `isCuratedSubstance()`, `isCustomSubstanceKey()`, `substanceLabel()` — the symptom vocabulary re-instantiated for the substance ledger (#3279); `substanceDef()` is total over the key space | `lib/substance-use.ts`                                                                              |
