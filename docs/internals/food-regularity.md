...
as *no expectation* — never as *habit broken*.
...
and returns what a tap would write — or `[]`, which is *no button at all*.
...
`food_log` is deliberately *not* a gated `STATEFUL_WRITE_TABLES` member (a second
...
frequency target, may be *measured* — the cadence ledger's own cap reporting is
entitled to the data — and is never *presented back as an expectation*.

Applied naively, regularity here produces *"you usually have alcohol in the
evening"*, which is a sentence this app does not say: #998 already settled that a
...
is about cap *semantics*, not disapproval.
