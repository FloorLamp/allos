...
Three surfaces render it: Trends → Nutrition's **Intake history** (food
servings) and **Dose history** (confirmed supplement/med doses), and Trends →
Fitness's **Workout history** (sessions by named activity, which replaced the
bespoke #186 `WorkoutHeatmap`).
...
5–15 confirms — the wider ladders keep them from saturating). A fourth domain
...
`trends-fitness-window.test.ts` (identity + windowing over real SQL), and the
Nutrition/Fitness specs in `e2e/` (rendering, the Timeline link shape, chip
filtering).
