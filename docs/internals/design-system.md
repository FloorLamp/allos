...
- **Guards are mandatory** (owner ruling 2026-08-21, recorded on #3459–#3501):
  ...
  test, or census probe that fails on regression. The cheap `components/**`
  test tier shipped in #3446; source-wide rules still live in `lib/__tests__`
  when the question is a census rather than one component's behavior.
...
| Links: `text-link` is the one inline action treatment; brand tone means interactive — static text never wears it, and no surface hand-rolls a link tone (sky included)                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | #2719; violations #3474, #3487, #3500 | shipped — `text-link-census.test.ts` scans the whole `app/` + `components/` corpus, proves it can see the retired hand-rolls, and stays quiet on the hover-only sibling                                                                                                                                                                          |
...
| Destination doors live on their owning surface: Standing rows name their tracked destination; medication dose/cabinet doors live in Today/Current medications; supplement doors live in Manage. Household's owner-approved cabinet + Illness episodes pair remains in `PageHeader.action` and shares the same trailing chevron vocabulary. A door label is the destination's own name; its chevron is decorative and hidden from assistive technology. Other arrow uses (row navigation, disclosure, carousel/pager controls) are explicitly classified as non-doors rather than swept into this rule                                                                                 | shipped — #3253, #3479, #3487, #3502                                                     | `lib/destination-door-grammar.ts` + `lib/__tests__/destination-door-grammar.test.ts`: the whole tracked `app/` + `components/` source corpus is compared with exact renderer/file/occurrence registries for four renderers, nine mounts, and every non-door chevron. Owning ancestry, destination/accessibility/child identity, static treatment, and decorative spacing are structural; spreads, duplicates, aliases, wrappers, re-exports, dynamic loads, and planted mutations fail closed. `lib/add-affordance-grammar.ts` continues to exclude `<Link>` so create placement and door placement remain separate questions |
...
| Tap floor                         | **44px EFFECTIVE target, everywhere** (owner ruling 2026-08-21, #3514) — met by either registered mechanism: rendered size (`min-h-11`) or a smaller rendered control extended to ≥44 effective by `.tap-target`'s hit-area overlay (#644: `inset: -6px` on a 32px icon ⇒ 44). Rendered height and hit area are DIFFERENT guarantees — a rule states which it means, never a bare "tap floor" number, and a control using NEITHER mechanism is the defect. **Converged**: the `.btn` family (rendered), `StarButton` and the two `IllnessNowGroup` controls (rendered), `CadenceEditor` weekday controls (rendered), the `OverflowMenu` kebab (including symptom-video actions) and the responsive-table variant (`.tap-target` / `inset: -6px`), `MENU_ITEM`'s coarse-pointer row. `MedicationCard`'s `min-h-10` stays — an `<li>` row container is layout rhythm, not a tap target | #644, #3377, ruled + shipped #3514                                                                                                                                                                                                                                                                                                                                                                     | the ≥44 e2e measurements (`button-height-floor`, `mobile-ui-polish`, `dashboard-now`, `trends-metric-pages`, `tap-target-census`). the source census of hand-rolled controls outside the family is `lib/tap-floor-reach.ts` + `lib/__tests__/tap-floor-reach.test.ts` (#3557); its corpus, governed-population and per-mechanism ratchets keep the scan honest without copying a transient exact count into this table. It also proves that `.tap-target` adds a FIXED 12px, so the hit-area mechanism only reaches 44 from a 32px rendered box up. #3562's routed 390px checks directly measure the repaired shell, quick-log, visit, fact, detail-menu, purpose, rule and cadence targets, including pairwise disjointness for adjacent rendered controls; its symptom-video test owns the standard one-overflow action geometry. The dormant unlocked `IntakeKindChip` branch was removed rather than licensed. |
...
| Design-guard suite                    | chip roles/selected state, action-sheet titles/row identity, link tones, and destination-door grammar                                                                                                  | shipped                      |
...
primitives (#3475, #3486 ✓ shipped, #3499 ✓ shipped, #3466 ✓ shipped, #3501, #3492 ✓ shipped, #3514 ✓ shipped) → idiom
...
The tap-floor reach is now measured and fail-closed, not complete. Its current
register still contains **53 under-floor controls** across dense editors, inline
glyphs, typed fields, bare native boxes, and one range track. Those are backlog,
not exceptions or compliance claims: `tap-floor-reach.test.ts` prevents the
number growing, requires every group to retain its reason, and goes stale when a
registered miss is repaired. #3562 closed the previously unreadable/unjudged
rosters and repaired its routed targets; it did not erase this separate register.
