...
| Custom named breakpoints use `rem`, so Tailwind can order them with its named breakpoints | #3477                   | `compiled-css-contracts.test.ts` (real PostCSS/Tailwind compile) |
...
| Series access                 | `SeriesSummary` / `SeriesPoint` (`components/SeriesAccess.tsx`): a custom strip or diagram carries its own data access — a visually hidden list of the same strings its marks encode, and each mark as a focusable target that names its value and shows it while focused or hovered (`series-point` in globals.css). No fold beside the picture; `VisualizationDetails` retired (#4760).                                                                                                                                         | shipped — #4760                                                                                                                                                                                                                  | `components/__tests__/series-access.test.tsx`                                                                                          |
...
`chip-filter` tokens outside `components/Chip.tsx`. The unique
...
There is no dense chip and no `density` prop: #3938 retired `chip-sm`'s
self-rendered 44px target, and what was left of it — `px-2.5` / `text-xs` — was a
second spelling of one control, which is the size-variant shape this system
forbids. A chip is a chip, at the control box. Timeline's category filters use the same typed
...
| **THE CONTROL BOX: 34px, every control kind, both viewports** (owner ruling 2026-08-28, #3938). It binds `chip-base`, the `.btn` family, `button-control`, typed fields and native selects, and it is the chip's MEASURED construction from #3475: content over a reserved 1px border, so a bordered and a borderless control occupy the same box. Declared ONCE in `app/globals.css` (SECTION: Touch tap targets) as `--control-box`, with the padding DERIVED from it and the element's own line box (`1lh`) — which is why `text-sm` leaves the chip's `py-1.5` and the `text-base` a typed field needs leaves `py-1`, both landing on 34, from one expression. The rule is UNLAYERED so it outranks a call site's own `py-*`; the icon-only floor beside it is in `@layer components` so a call site asking for MORE still wins. What it does NOT reach is a control outside those selectors: `IconButton`, `StarButton` and `DoseStatusControl`'s circles wear `--control-box` themselves; `SegmentedControl`, `checkbox-control` and `Combobox`'s option rows still render their own 44 | shipped — #3938; supersedes #3486/#3514's rendered halves and #3708 | `e2e/button-height-floor.mobile.spec.ts` — rendered bounding boxes at 390/639/640/1280, never a class string, plus the ledger's mixed chip row; `lib/__tests__/phone-only-compiled-css.test.ts` pins WHICH LAYER each half compiles into; the star/log pair on `/trends/metric/weight` is pinned EQUAL in `e2e/trends-metric-pages.spec.ts` |
| **Typed fields wear the control box, at every width, with a ≥16px line.** The BOX is the target — a labeled row beside it may not stand in for it, and there is no per-site exception (owner ruling #3708, restated at the one box by #3938). `.input` carries `text-base` because a focused control under 16px makes iOS Safari zoom the page; the derived padding turns that taller line into `py-1` and the field lands on the same 34 as everything else. The `h-8`/`h-9`/`h-[38px]` pins the #3502 reconciliation named are DELETED rather than outranked — a call-site height is the drift, not the fix — while `min-h-16`/`min-h-20` textareas keep theirs. A typed `<input>` and a native `<select>` render no pseudo-element, so their target IS the box; that is the narrowing #3938 made to #3514                                                                                                                                                                                                                                                                                  | owner ruling #3708, superseded by #3938; adopters #3706, #3709      | `e2e/tap-target-census.mobile.spec.ts` — routed rendered boxes at 390/639/640/1280 (equality against the box, viewport containment, pairwise disjointness, focus, accessible name, editing behaviour). The old pair of inequalities was retired because 34 satisfies "< 44": a boundary-crossing fixture goes quiet rather than red         |
...
| Tap floor                         | **44px EFFECTIVE target, and effective ONLY** (#3514, narrowed by #3938). Every control renders the 34px box; a coarse pointer gets `--control-reach` (6px per side) back around it, so 34 + 2×6 = 46 clears the floor, and adjacent targets keep a gap floor of TWICE the reach so their hit regions stay disjoint — `FilterPills` and the ledger's chip row spend `gap-3` on exactly that. WHERE REACH REPAIR IS IMPOSSIBLE THE BOX IS THE TARGET: a native `<select>` and a typed `<input>` render no pseudo-element (the retired `chip-sm`'s lesson). Migrating the ~100 native selects to an owned picker would recover the floor and MAY happen opportunistically; it is not a prerequisite. `MedicationCard`'s `min-h-10` stays because the `<li>` is layout rhythm rather than a tap target. | `lib/tap-floor-tokens.ts` owns shared browser-proof geometry. Primitive component tests and rendered mobile geometry specs prove registered owners; #3706–#3709/#3719 track the remaining migrations. There is no global source-compliance claim.                                                                                                                                                      |
