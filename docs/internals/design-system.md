...
Dense in-row strips request `density="dense"` from `Chip`; since #3938 that is
HORIZONTAL and TYPE density only (`px-2.5` / `text-xs`) — a dense chip is the same
34px control box as everything else, and its self-rendered 44px target retired with
the rest of them. Regular chips omit the density prop. Timeline's category filters use the same typed
