...
| Chip / pill                   | `components/Chip.tsx` owns the two declared roles #3408 ruled: navigation (`chip-nav` — rounded-full outline) and filter (`chip-filter` — soft tinted well, `rounded-md`, active fill). Its typed API admits only regular/dense geometry and derives `aria-current` or `aria-pressed` from current/pressed state; callers cannot choose classes, paint, shape, or independent selected-state ARIA. Dense chips render `min-h-11`; regular scrolling strips retain the reviewed width-acquisition treatment. `FilterPills` remains the one record-list filter composition, and `SegmentedControl` remains the mutually-exclusive view primitive. | shipped — #3724 converged the registered call sites, including Timeline's category filters; typed fields and non-chip selected controls remain their own questions | `components/__tests__/chip.test.tsx`, small raw-token residual `lib/__tests__/chip-residual.test.ts`, and `e2e/records-pane-anatomy.mobile.spec.ts` (rendered roles, size, shape, and selected paint) |
...
props. `FilterPills` composes the primitive with a labelled scrolling group and
requires the whole group to choose link mode or button mode; it cannot mix both.

The residual guard rejects literal and hoisted raw chip tokens outside
`components/Chip.tsx`. It has no adopter list or class-expression interpreter.
`DateRangeControl` may supply Timeline's pending/scroll-restoring link renderer,
but `Chip` still supplies that renderer's presentation and current state.
...
| Tap floor                         | **44px EFFECTIVE target, everywhere** (owner ruling 2026-08-21, #3514) — met by rendered size (`min-h-11`) or a smaller rendered control extended to ≥44 effective by `.tap-target` (`inset: -6px` on a 32px icon ⇒ 44). Rendered height and hit area are different guarantees. **Partial, not globally converged:** `IconButton`, `Chip` dense mode, the button family, overflow actions, and the named rendered/overlay adopters own their floor; the source census still records unresolved dense controls, typed-field questions, bare boxes, and unreadable seams under #3706–#3709/#3719/#3720. `MedicationCard`'s `min-h-10` stays because the `<li>` is layout rhythm rather than a tap target. | #644, #3377, ruled + shipped #3514; remaining work tracked by #3714 and its linked issues                                                                                                                                                                                                                                                                                                              | `components/__tests__/icon-button.test.tsx`, `components/__tests__/chip.test.tsx`, rendered mobile geometry specs, and `lib/tap-floor-reach.ts` + `lib/__tests__/tap-floor-reach.test.ts`. The source census proves only the source shapes it can read and keeps its unresolved registers explicit; it is not a universal compliance claim. |
