...
| Four governed destination-door renderers live on their registered owning surfaces: Standing rows name their tracked destination; medication dose/cabinet doors live in Today/Current medications; supplement doors live in Manage. Household's owner-approved cabinet + Illness episodes pair remains in `PageHeader.action` and shares the same trailing chevron vocabulary. Each governed door uses its destination identity and a decorative `IconChevronRight`. This contract does not classify unrelated `IconArrowRight` or text-arrow affordances, including Training's “Open registry →” copy.                                                                                | shipped — #3253, #3479, #3487, #3502                                                     | `lib/destination-door-grammar.ts` + `lib/__tests__/destination-door-grammar.test.ts`: every tracked runtime source is checked against exact registries for four renderers, nine mounts, and each current `IconChevronRight` occurrence. Renderer provenance, live return paths, mount props/owners, destination/accessibility identity, and treatment are structural; unsupported indirection or overrides fail closed. `lib/add-affordance-grammar.ts` continues to exclude `<Link>` so create placement and governed door placement remain separate questions |
