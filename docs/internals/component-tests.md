...
(`<NotesText>`, the e2e-hygiene guard, the responsive-surface convention, …),
and **high-logic is extracted into pure `lib/` functions that carry unit
tests**. We do not add jsdom + testing-library.
...
- **The load-bearing logic is already extractable — and the convention is to
  ...
  belongs in `lib/` where the pure tier already covers it. A component tier
  would invite leaving that logic _in_ the component, weakening the extraction
  discipline.
- **A jsdom tier is a new, permanently-carried cost** — another runner, another
  ...
  that drift from the real Next Server/Client boundary (Server Components,
  Server Actions, `next/*` mocks). Playwright already renders the _real_ tree
  against the _real_ app, which is the higher-fidelity signal for "does this
  component paint."
...
deliberately, with the same reasoning, not as a side effect of one hard
component.
