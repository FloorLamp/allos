...
- The verdict authorises the RESCUE, never the RELAUNCH. Both detectors are
  proxies, and a snapshot-style resume changes both ids while the process tree
  survives — so confirm with `ListAgents` before relaunching anything. Rescuing
  a live agent's tree costs a junk commit; relaunching onto one puts two writers
  on a worktree.
