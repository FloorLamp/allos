...
- **Attached means on the GitHub comment** (owner, 2026-09-05): the API takes
  no uploads, so the orchestrator pushes the PNGs to the orphan `screenshots`
  branch under `<issue>/` with `[skip ci]` and embeds each by its raw URL.
  Never a session, an artifact page or an external host; only the owner deletes.
