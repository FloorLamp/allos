...
- Use `scripts/work/dispatch-brief.mjs new` for every agent; adopt any
...
- Claim before dispatch: a `Dispatched:` note on each issue, and a fence
  check against any other worker's branches (`multi-worker.md`).
  The note outlives the container, so it applies with one worker too.
...
- Lanes never file issues. Findings ride the return summary; the worker
...
- Worker bookkeeping in `lib/release-notes.json`: one batch a day at
