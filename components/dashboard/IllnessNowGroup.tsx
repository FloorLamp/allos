      ...
      {cockpits.map((c) => {
        ...
        return (
          <div
            ...
            // A READABLE MEASURE, AND ITS HOST IS CURRENT CARE (#4752 item 2,
            // reconciled by #5490 site 3, unhosted by #5435 §4, re-hosted by #5894's
            // owner ruling 2026-09-15 option (a)). The cockpit was a phone layout
            ...
            // §4 deleted that band and Home mounts this group directly, which is how
            // the measure lost its declarer and the row ran to the page container's
            // width. It was not repaired here, because re-capping the row would restore
            // the stepped edges #5490 removed — the measure needed a host, not a
            // relapse, and Home's Current care BLOCK is that host: it carries
            // `current-care-measure` (app/globals.css) and frames this group and the
            // Reopen list alike, so both keep one left edge. What this still keeps
            // either way is its own gutter (the group's row supplies none for it) and
            // the rail and fill that say someone is ill right now.
