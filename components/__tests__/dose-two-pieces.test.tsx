...
describe("the PRN row states the dose it logged (#5663/#5900)", () => {
  ...
  it("drops the receipt when the day or the subject moves", async () => {
    // A live day, so the tap writes rather than asking for the minute.
    ...
    view.rerender(<QuickLogPrnControl {...props} date={PREV_DAY} />);
    expect(screen.queryByTestId("prn-receipt")).toBeNull();

    view.rerender(<QuickLogPrnControl {...props} date={NOW_DAY} />);
    await act(async () => fireEvent.click(screen.getByTestId("prn-log-now")));
    expect(screen.getByTestId("prn-receipt")).toBeTruthy();
    view.rerender(
...
// THE FOLD'S TAKE POSTS THE DATED CORE (#5808, owner ruling 2026-09-10).
//
// This is the claim the whole row rests on, and the one a copy would get wrong:
// `markDoseTaken` resolves an OCCURRENCE, and the items this fold exists for — an
// unscheduled supplement above all — have none. So the write is `logHistoricalDose`,
// the record door's own core, with the sheet's day and a stated minute.
//
// AND THE MINUTE IS THE OTHER HALF. The day is the surface's (#4738 ruling 1); the
// time is #4426's collapsed statement in its usual two pieces. On the live day the tap
// states the gather's server minute so it stays one tap; on a day that has ended there
// is no "now" to state, so the tap ASKS instead of inventing one.
describe("the quick sheet's fold writes through the dated core (#5808)", () => {
  const OTHERS = {
    nowHhmm: "09:30",
    byDate: {
      [TODAY]: [
  ...
  it("states the gather's minute on the live day, in one tap", async () => {
    renderFold();
    await act(async () => {
      fireEvent.click(screen.getByTestId("quick-entry-other-take-71"));
    });

    expect(mocks.logHistoricalDose).toHaveBeenCalledTimes(1);
  ...
  it("asks for the minute on a day that has ended, then writes that day", async () => {
    renderFold();
    fireEvent.click(screen.getByRole("button", { name: "Yesterday" }));

    // FIRST TAP WRITES NOTHING. Yesterday has no "now", and the instant is the record;
    // guessing one would file a dose at a minute nobody stated.
    await act(async () => {
      fireEvent.click(screen.getByTestId("quick-entry-other-take-71"));
    });
    expect(mocks.logHistoricalDose).not.toHaveBeenCalled();

    const time = screen.getByTestId("quick-entry-other-when-71-time");
    await act(async () => {
      fireEvent.change(time, { target: { value: "20:05" } });
    });
    await act(async () => {
      fireEvent.click(screen.getByTestId("quick-entry-other-take-71"));
    });
    ...
    expect(fields()).toMatchObject({
      id: "71",
      dose_id: "171",
      // The day the surface was standing on — not today, and not the day the tap
      // happened to land on.
      date: YESTERDAY,
      time: "20:05",
    });
