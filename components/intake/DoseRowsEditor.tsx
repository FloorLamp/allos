...
export default function DoseRowsEditor({
  ...
  const doseRows = useRowList(setDoses);
  ...
  return (
    <RowRepeater
      rows={doses}
      header={<div className="mb-2 section-label">Doses</div>}
      add={{
        label: "Add dose",
        onClick: () =>
          onAddDose ? onAddDose() : doseRows.add(emptyDose(newDoseAmount)),
      }}
    >
      {doses.map((d, i) => (
        <div
        ...
        >
          <Combobox
            ariaLabel="Amount"
            value={d.amount}
            onChange={(v) => doseRows.patch(i, { amount: v })}
            options={dosageOptions}
            allowFreeText
            placeholder={amountPlaceholder}
          />
          <select
            ...
            onChange={(e) => doseRows.patch(i, { time_of_day: e.target.value })}
            className="input"
          ...
          {!hideFoodTiming && (
            <select
              value={d.food_timing}
              onChange={(e) =>
                doseRows.patch(i, { food_timing: e.target.value as FoodTiming })
              }
              className="input"
              aria-label="Food timing"
            >
          ...
          >
            ...
            <div className="mt-2 space-y-2 border-l-2 border-black/10 pl-3 dark:border-white/10">
              <WeekdayChips
                value={d.weekdays}
                onChange={(weekdays) => doseRows.patch(i, { weekdays })}
                weekStart={weekStart}
                idPrefix={`dose-${i}`}
              />
              <div className="grid gap-2 sm:grid-cols-2">
                <label className="text-xs text-slate-500 dark:text-slate-400">
                  ...
                  <DateField
                    value={d.start_date}
                    onChange={(start_date) => doseRows.patch(i, { start_date })}
                    inputClassName="mt-1"
                ...
                <label className="text-xs text-slate-500 dark:text-slate-400">
                  ...
                  <DateField
                    value={d.end_date}
                    onChange={(end_date) => doseRows.patch(i, { end_date })}
                    inputClassName="mt-1"
