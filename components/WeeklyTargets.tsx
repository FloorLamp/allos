...
// rep, filled by sessions logged) inside a status-coloured border —
// emerald = met, amber = in progress, rose = not started. Becomes a button when
// `onClick` is given (e.g. to select it for editing).
export function WeeklyTargetChip({
  target: { label, count, perWeek, met },
  onClick,
  selected,
}: {
  target: WeeklyTarget;
