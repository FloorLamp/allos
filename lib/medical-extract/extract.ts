...
export function describeError(err: unknown): string {
  ...
  if (err instanceof APIError) {
    const s = err.status;
    if (s === 401 || s === 403)
      // The reader's half, not the operator's (#3852): the rejection is ours,
      // their file is intact, and re-upload is the recovery 429 already names.
...
export function resultFromExtractionInput(
  ...
  return {
    ...
    opticalPrescriptions: clinical.opticalPrescriptions,
...
export function clinicalCountOf(r: ExtractionSuccess): number {
  return (
    ...
    (r.opticalPrescriptions?.length ?? 0) +
