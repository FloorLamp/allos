...
export const NON_MARKER_NOTIFY_KEYS: readonly {
  ...
  },
  {
    ...
    what: "what the delivered \"Still working out?\" nudge PROPOSED for one open workout row (#5194): `notify_end_proposal_workout_<activityId>` holds the profile-local `HH:MM` the message quoted, or `-` when it quoted none, written ON DELIVERY by lib/notifications/still-going.ts in the same branch as the notify_stale_workout_ marker above and immediately before it — where delivery means at least one RECIPIENT received the message (DispatchResult.delivered), not that the channel reported success, because Telegram fans one message out to a household and fails whole when any one chat has blocked the bot — and read by finishWorkoutSession when the person taps Finish. It gates NO send — the nudge's one message per row is deduped by that marker — it decides what that person's own tap WRITES, and only ever downwards: the detector's save cancel is re-applied to the recorded minute at tap time, so a person who resumed their workout stamps their own instant instead. It exists because the quoted minute is unrecoverable afterwards: re-asking the detector at tap time answers about the trace as it is and never as it was, so a message naming 16:35 stamped 18:30 the moment a single later minute rose above the resting ceiling. Retention: id-keyed, spent by the finish or discard that resolves the row through lib/workout-finish.ts, and otherwise a permanent dead setting on the same terms as the marker beside it (#203)",
  },
