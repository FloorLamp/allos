...
//
// Every kind in the `NotificationKind` union is accounted for here: it has a row, or
// it says why it has none. `NOTIFICATION_KINDS` is KEYED on the union, so that
// partition is a compile-time fact and not something a test reconciles.

import type { NotificationKind } from "./types";
import type { ReminderWindow } from "./intake-format";

// THE SAFETY TIER (#928): scheduled dose reminders, missed-dose escalation, and the
// PRN redose notice. A channel may disable them, but the UI WARNS — never blocks —
// when one ends up off on EVERY configured channel, consistent with the findings-bus
// principle that a safety signal is never silently suppressed.
//
// SAFETY IS A PROPERTY OF THE KIND, not a flag a row repeats. It used to be both — a
// `safety: boolean` on every registry row plus `redose` re-added by hand here, because
// `redose` has no row — which is two copies of one fact and a guard to keep them in
// step. This list is the fact; `NOTIFICATION_KINDS` reads it in its own type.
const SAFETY_KINDS = [
  "dose",
  "escalation",
  "redose",
...
/** A kind with NO settings row, and the reason it has none. */
export type NonConfigurableKind = { readonly inert: string };

/**
 * THE ONE REGISTRY, KEYED ON THE UNION IT COVERS (#5351/#5346). Every `NotificationKind`
 * appears exactly once: it has a settings row, or it says why it has none. That used to
 * be an ARRAY plus a partial exclusion map plus a hand-written copy of the union, with
 * `lib/__tests__/notification-kinds.test.ts` reconciling the three on every run — a
 * guard that lists a union's members does not track the union
 * (docs/internals/verification-failure-modes.md). Keyed this way, `tsc` tracks it: a new
 * kind fails to compile until it is decided about, a retired one fails until it is
 * removed, and a duplicate cannot be written down.
 *
 * A SAFETY KIND'S ROW MAY ONLY BE `control: { type: "always" }`. The thing that
 * generates the send — a dose schedule, an administration's redose interval — is the
 * only off-switch there is, and a per-kind checkbox beside it would be a second one that
 * an episode planner's freeze rule could then reach. That is now the row's TYPE rather
 * than a rule a test restates.
 *
 * The registry is PRESENTATION + CONFIG SHAPE only. It changes no delivery semantics.
 */
export const NOTIFICATION_KINDS: {
  readonly [K in NotificationKind]:
    | (K extends SafetyKind
        ? NotificationKindEntry & { control: { type: "always" } }
  ...
  test: {
    inert:
      "A send-test from Settings — always delivered, so the user can verify the wiring.",
  },
  other: { inert: "The internal catch-all for an unclassified message." },
  upcoming: {
    inert:
  ...
  temp: {
    inert:
      "The reply prompt for a `/temp` command (#859) — same reasoning as the other on-demand replies: the request IS the consent.",
  },
  "practice-list": {
    inert:
...
/** Every value of the NotificationKind union, as data — the registry's own keys. */
export const ALL_NOTIFICATION_KINDS = Object.keys(
  NOTIFICATION_KINDS
) as readonly NotificationKind[];

/** A settings row together with the kind it belongs to. */
export type NotificationKindRow = NotificationKindEntry & {
  kind: NotificationKind;
};

/**
 * The rows the Notifications settings page renders, in declaration order — the
 * configurable half of the registry above.
 */
export const NOTIFICATION_KIND_REGISTRY: readonly NotificationKindRow[] =
  Object.entries(NOTIFICATION_KINDS).flatMap(([kind, entry]) =>
    "inert" in entry ? [] : [{ kind: kind as NotificationKind, ...entry }]
  );

/** The kinds with no settings row, and why — the other half, for the docs. */
export const NON_CONFIGURABLE_KINDS: Readonly<
  Partial<Record<NotificationKind, string>>
> = Object.fromEntries(
  Object.entries(NOTIFICATION_KINDS).flatMap(([kind, entry]) =>
    "inert" in entry ? [[kind, entry.inert]] : []
  )
);
...
export function notificationKindEntry(
  kind: NotificationKind
): NotificationKindRow | undefined {
  const entry = NOTIFICATION_KINDS[kind];
  return "inert" in entry ? undefined : { kind, ...entry };
}
