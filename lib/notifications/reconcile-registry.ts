...
// Which tokens remain INERT for this pass. Claim-view controls are inert while their
// claims are live, then expire with them; standalone controls remain inert. An UNKNOWN
// prefix is deliberately NOT inert:
...
export function inertTokens(
  ...
  for (const t of tokens) {
    ...
    if (entry?.inert && (!claimsExpired || entry.expiry === "standalone"))
      out.add(t);
  }
  return out;
}
