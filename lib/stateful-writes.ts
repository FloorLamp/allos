...
export const STATEFUL_WRITE_TABLES: readonly StatefulWriteTable[] = [
  ...
  },
  {
    ...
    why: "#2756: a fast is an open/closed LIFECYCLE row on the `cycles` shape — `ended_at IS NULL` IS the active state, and the one-active-per-profile invariant is what every derivation downstream assumes cannot be violated, the #2757 notification stand-down included. lib/fast-write.ts is the auth-blind core: one writeTx per transition (start / end / reopen-as-Undo / discard / edit), every refusal typed (already-active including the cross-device double-start, none-active, end <= start, and a backdated interval that overlaps an existing fast) and enforced with the SAME pure predicates the Nutrition control renders from (fastControlState, lib/fasting.ts). It reaches the table only through lib/fast-store.ts, which holds the DML. A raw write from a third module could mint a second simultaneously-active fast — precisely the state the partial unique index and every reader rule out — or, worse, bypass the life-stage gate the core carries (ADULT_ONLY_WRITE_CORES), which is a safety refusal and not a preference. Not column-narrowed: the table has no non-lifecycle column — `started_at`/`ended_at` ARE the machine and `note` never travels alone.",
  },
  {
