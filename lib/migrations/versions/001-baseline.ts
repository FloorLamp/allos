...
import { tableColumns, runBootTx } from "../schema-utils";
import {
  renameAuthTablesForBranch,
  migrateWeighInsToBodyMetrics,
  migrateSupplementsToIntakeItems,
} from "../renames";
import { migrateMedicationHistory } from "../intake";
import {
  rebuildForProfileScoping,
  rebuildMetricSamplesSourceKey,
  relaxBodyMetricsWeightKg,
  swapProfileScopedIndexes,
} from "../profile-scoping";
import { reconcileEnumChecks } from "../enum-checks";
import type { Migration } from "../runner";

// -----------------------------------------------------------------------------
// MIGRATION 001 - BASELINE (issue #119). FROZEN - DO NOT EDIT.
//
// This is the entire pre-runner `migrate()` body, MOVED here verbatim and frozen
// as the first versioned migration. Existing deployed DBs are at
// `PRAGMA user_version = 0`; they replay this once (identical to every boot they
// already performed) and get stamped to 1. Fresh DBs replay the same single path,
// so fresh/upgraded schema divergence stays impossible by construction.
//
// APPEND-ONLY: never edit a shipped migration (the hash manifest fails CI). A new
// column is a plain `ALTER TABLE` in a NEW `NNN-*.ts`; a grown enum CHECK is a
// rebuild migration; a one-shot data move is its own migration. The historical
// mechanisms below (`addColumnIfMissing`/`ADDITIVE_COLUMNS`, the `ENUM_CHECKS`
// reconcile, settings-flag one-shots, the rename shims) are kept exactly as they
// were - grandfathered, and closed to NEW entries (a lint-style pure test).
//
// WHAT WAS LIFTED OUT (into lib/migrations/boot-tasks.ts, run on EVERY boot AFTER
// the runner, NOT frozen here): `bootstrapAuth`, `backfillProfileIds`,
// `seedCanonicalBiomarkers`, `reconcileFlagsIfCanonicalChanged`, the stuck-state
// cleanups, and `seedTimezoneFromEnv`. Their call sites were removed from the body
// below; the surrounding comments (which still narrate the old inline order -
// e.g. "after bootstrapAuth() has ensured profile 1 exists") are preserved
// verbatim as historical narration. On the paths that actually run today (a fresh
// DB, or a fully-migrated DB at user_version 0, or the migrate.test.ts
// reconstruction which keeps `profile_id`), the profile-scoping rebuilds below are
// no-ops (their `!includes("profile_id")` guards short-circuit), so replaying this
// body without profile 1 yet existing is safe; the per-boot tasks then create /
// adopt profile 1 immediately afterward. See boot-tasks.ts for the rationale of
// each lifted task.
// -----------------------------------------------------------------------------
...
// The baseline schema build. See the header: the frozen pre-runner `migrate()`
// body minus the per-boot tasks. Runs inside the runner's IMMEDIATE transaction
// (its internal `runBootTx`/`db.transaction` calls nest as savepoints).
