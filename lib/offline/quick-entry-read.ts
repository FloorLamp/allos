...
//      Food tallies) mapped onto the form's payload with pending intents folded in.
//      For the two declared flows with no snapshot kind (mood, stool), it uses what
//      the device itself knows: the day and its own queued taps. Everything else is a
//      miss, and the host renders the retry state.
//
...
import { foodGroupBySlug, type FoodGroup } from "@/lib/food-groups";
import { FOOD_SLOTS, foodSlotForHhmm, type FoodSlot } from "@/lib/food-slot";
...
export function subscribeLastGoodInvalidation(
  listener: () => void
): () => void {
...
export interface OfflineQuickEntry {
  ...
  // five forms. With a `fetchedAt` it is the `why` half of the #2908 as-of line
...
function servingSnapshot<
  K extends SnapshotKind & ("dose-schedule" | "food-tallies" | "practice-week"),
>(
 ...
 * else would be a door onto a refusal. Dose and practice have subject-aware writes,
 * while Food's queue is acting-profile-only, so its otherwise profile-scoped snapshot
 * must keep the same acting-profile bound as Mood and stool.
 */
  ...
  switch (form) {
    ...
    case "mood": {
      ...
      const queued = mine.findLast(
        (intent) => intent.flow === "mood" && intent.date === parts.day
      );
    ...
    case "food": {
      if (parts.profileId !== actingProfileId) return null;
      const env = servingSnapshot(snapshots, "food-tallies", parts);
      if (!env?.data.quickEntry) return null;
      const quickEntry = env.data.quickEntry;
      ...
      // Fold the queue into this snapshot exactly once before mapping the server's
      // persisted quick-entry facts onto the real FoodLogBar payload.
      const overlaid = overlayFoodTallies(env.data, mine, parts.profileId);
      const available = overlaid.quickEntry;
      if (!available?.available) return null;
      const groupsBySlot = Object.fromEntries(
        FOOD_SLOTS.map((slot) => [
          slot,
          available.rankedGroupSlugsBySlot[slot].flatMap((slug) => {
          ...
          days: [
            {
              ...
              counts: Object.fromEntries(
                overlaid.groups.map((group) => [group.key, group.servings])
              ),
              slotCounts: available.slotCounts,
              // A food-tallies snapshot has no event ledger. The real FoodLogBar
              // therefore omits correction rows instead of inventing event identity.
              events: [],
            },
          ],
          groupsBySlot,
          proteinRankBySlot: available.proteinRankBySlot,
          proteinGrams: overlaid.proteinGrams ?? 0,
          proteinPreset: available.proteinPreset,
          excludedGroups: available.excludedGroups,
          slot: foodSlotForHhmm(
            zonedDateParts(env.timeZone, now).hhmm,
            available.slotBoundaries
          ),
          slotBoundaries: available.slotBoundaries,
        },
      };
    }
    // The rest have no offline story at all (LOG_MANIFEST's `offline` column) or
    // gather nothing worth standing in for.
    case "cycle":
    case "substance":
    case "symptom":
    case "document":
