...
// it left behind — `illness_type = 0` with `active = 1` — had no other producer and no
// meaning: the situation literally named Illness, declared not to be one. (Nothing in the
// tree mounts a control for it either; toggleSituationIllnessType ships with no caller
// but a test, so the state was reachable without ever being offered.) It did have a
// consequence: `getIllnessSituations` stopped seeing the container while
// `getActiveSituations` still saw the situation, so the symptom card's "Mark as illness"
// bridge and the fever offer's "Open an episode" both drew a door onto a container that
// no longer existed — and re-activating could not rebuild one, because the whole-set
...
// would not notice a second module learning to write this column. It holds today only
// because this is the single production write that can CHANGE an existing row's flag:
// the other two writers, migration 042's backfill and resolveSituationId's INSERT, each
// set a born row's value and never revisit it.
