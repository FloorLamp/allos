...
// re-export (#5852). It has TWO limits, and the second is why it is not the whole barrier:
// it matches the brand BY NAME rather than chasing what a name resolves to, and it does not
// reach every production file. lib/revalidate.ts is named in the `ignores` of all three
// blocks carrying the ban, and the repo-root modules (middleware.ts, instrumentation-client.ts)
// sit outside PRODUCTION_TREES, so those three keep only the ten temporal selectors and a cast
// there is unrefused — filed as #5856, open. lib/, app/, components/ and scripts/ ARE covered,
// and `tsc` refuses the accidental ungated call everywhere regardless of any of this.
...
// RPE_BRAND_CAST makes. The import is type-only —
