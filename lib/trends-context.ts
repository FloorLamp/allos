...
// exactly names the window by its own short label; the open window is "All time"; anything
...
export function activeRangeLabel(range: DateRange, todayStr: string): string {
  const match = quickRanges(todayStr).find((qr) =>
