...
// simple `(date, type)` grouping by copy; the ADJACENT-DAY widening below is subtle
// enough that a second copy of it would be a bug waiting to happen, so it lives
// here and both interpolate it.
//
...
/**
 * The adjacent-day candidate pairs (#2056): two CROSS-SOURCE activities of one type
 * on consecutive days, the earlier starting late enough in the evening and the later
 * early enough in the morning that ONE session pushed across midnight by a wrong UTC
 * offset is the plausible reading.
 *
 * A row set, not a filter: it yields the (date, type) bucket of BOTH sides, so the
 * loaders can union them into the bucket pre-filter and hand the pure detector rows
 * it can actually pair. The narrowness that matters is downstream — the classifier
 * still demands an offset-SHAPED start gap and agreement on both duration and
 * distance — so this only has to be bounded, and the near-midnight window is what
 * bounds it.
 *
 * Binds, in order: `profile_id`, the evening clock threshold, the morning one.
 */
export const ACTIVITY_MIDNIGHT_CANDIDATE_SQL = `
  SELECT e.date AS evening_date, l.date AS morning_date, e.type AS type
    FROM activities e
    JOIN activities l
