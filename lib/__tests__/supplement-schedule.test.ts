...
describe("isDueOn", () => {
  ...
  it("workout-linked conditions follow the workout-day flag", () => {
    ...
    expect(isDueOn(pre, ctx({ date: "2026-03-04", isWorkoutDay: true }))).toBe(true);
    expect(isDueOn(pre, ctx({ date: "2026-03-04", isWorkoutDay: false }))).toBe(false);
    expect(isDueOn(post, ctx({ date: "2026-03-04", isWorkoutDay: true }))).toBe(true);
  });
  ...
  it("pre_workout is due on a predicted workout day with NO logged activity", () => {
    const pre = { condition: "pre_workout" as const, situation: null };
    expect(
      isDueOn(pre, ctx({ date: "2026-03-04", isWorkoutDay: false, predictedWorkoutDay: true }))
    ).toBe(true);
    expect(
      isDueOn(pre, ctx({ date: "2026-03-04", isWorkoutDay: false, predictedWorkoutDay: false }))
    ).toBe(false);
    // predictedWorkoutDay wins over the logged flag when known.
    expect(
      isDueOn(pre, ctx({ date: "2026-03-04", isWorkoutDay: true, predictedWorkoutDay: false }))
    ).toBe(false);
    // null → fall back to the logged flag (old behavior).
    expect(
      isDueOn(pre, ctx({ date: "2026-03-04", isWorkoutDay: true, predictedWorkoutDay: null }))
    ).toBe(true);
  });
  ...
  it("post_workout needs a logged session and its end time", () => {
    ...
    expect(
      isDueOn(post, ctx({ date: "2026-03-04", isWorkoutDay: false, predictedWorkoutDay: true }))
    ).toBe(false);
    // Logged but session not over yet → not due.
    expect(
      isDueOn(post, ctx({ date: "2026-03-04", isWorkoutDay: true, postWorkoutReady: false }))
    ).toBe(false);
    // Logged and session over → due.
    expect(
      isDueOn(post, ctx({ date: "2026-03-04", isWorkoutDay: true, postWorkoutReady: true }))
    ).toBe(true);
  });

  it("rest_day is the inverse of a workout day", () => {
    const rest = { condition: "rest_day" as const, situation: null };
    expect(isDueOn(rest, ctx({ date: "2026-03-04", isWorkoutDay: false }))).toBe(true);
    expect(isDueOn(rest, ctx({ date: "2026-03-04", isWorkoutDay: true }))).toBe(false);
  });
  ...
  it("rest_day follows the predicted signal when known", () => {
    const rest = { condition: "rest_day" as const, situation: null };
    expect(
      isDueOn(rest, ctx({ date: "2026-03-04", isWorkoutDay: false, predictedWorkoutDay: true }))
    ).toBe(false);
    expect(
      isDueOn(rest, ctx({ date: "2026-03-04", isWorkoutDay: true, predictedWorkoutDay: false }))
    ).toBe(true);
  });
...
describe("situational dueness survives the isDueOn/isOfferedOn split (#1505)", () => {
  ...
  const ctx = (activeSituations: string[]) => ({ date: "2026-03-04",
    isWorkoutDay: false,
