...
function projectStaticRenderedCopy(source: string): ProjectedText[] {
  ...
  const collectBindings = (node: ts.Node): void => {
    if (ts.isVariableDeclaration(node)) {
      ...
      for (const binding of bindingEntries(
        node.name,
        [],
        node.initializer ? [{ initializer: node.initializer, depth: 0 }] : []
      )) {
        bindings.push({
          ...binding,
          scope: node.parent,
  ...
  type InputValue = {
    ...
    projections: ProjectedText[];
    definitelyDefined?: boolean;
    scanAsProp?: boolean;
  };
  type RenderInput =
  ...
  const lexicalThisInputs = new Map<ts.ArrowFunction, RenderInput>();
  const activeGetterReads = new Set<ts.GetAccessorDeclaration>();
  const expressionInput = (expression: ts.Expression): RenderInput => ({
    kind: "value",
  ...
  const thisInput = (node: ts.Node): RenderInput | null => {
    for (let current = node.parent; current; current = current.parent) {
      if (ts.isArrowFunction(current)) {
        const lexical = lexicalThisInputs.get(current);
  ...
  const receiverInput = (expression: ts.Expression): RenderInput =>
    expression.kind === ts.SyntaxKind.ThisKeyword
      ? (thisInput(expression) ?? expressionInput(expression))
      : expressionInput(expression);
  const withThisInput = <T>(
    node: ts.FunctionLikeDeclaration,
    receiver: RenderInput,
    callback: () => T
  ): T => {
    const previous = activeThisInputs.get(node);
    activeThisInputs.set(node, receiver);
    try {
      return callback();
    } finally {
      if (previous) activeThisInputs.set(node, previous);
      else activeThisInputs.delete(node);
    }
  };
  const withGetterRead = <T>(
    node: ts.GetAccessorDeclaration,
    fallback: T,
    callback: () => T
  ): T => {
    if (activeGetterReads.has(node)) return fallback;
    activeGetterReads.add(node);
    try {
      return callback();
    } finally {
      activeGetterReads.delete(node);
    }
  };
    ...
    if (ts.isObjectLiteralExpression(expression)) {
      for (const property of [...expression.properties].reverse()) {
        ...
        if (
          ...
          return withGetterRead(property, null, () =>
            withThisInput(property, expressionInput(expression), () => {
              const [returned] = functionReturnExpressions(property);
              return returned
                ? staticExpressionAtPath(returned, tail, seen)
                : null;
            })
          );
        }
  ...
  function staticPathPresent(
    ...
    const [head, ...tail] = path;
    if (ts.isCallExpression(expression) && typeof head === "string") {
      const states = objectPropVariants(expression).map((props) => {
        const value = props.get(head);
        if (!value) return false;
        if (tail.length === 0) return true;
        const nested = value.expressions.map((candidate) =>
          staticPathPresent(candidate, tail, seen)
        );
        return nested.length > 0 && nested.every((state) => state === nested[0])
          ? nested[0]
          : null;
      });
  ...
  const inputValueAt = (
    input: RenderInput,
    path: StaticPath
  ): { value: InputValue; tail: StaticPath } | null => {
    if (input.kind === "value") return { value: input.value, tail: path };
    if (path.length === 0) {
      return {
        value: { expressions: [], projections: [], definitelyDefined: true },
        tail: [],
      };
    }
    const [head, ...tail] = path;
  ...
  type BoundRenderInput = { input: RenderInput; pathOffset: number };
  const bindingRenderInput = (
    binding: LexicalBinding,
    input: RenderInput | null
  ): BoundRenderInput | null => {
    if (!input) return null;
    if (!binding.objectRest) return { input, pathOffset: 0 };
    if (input.kind !== "props") return { input, pathOffset: 0 };
    const rest = binding.objectRest;
    if (rest.path.length === 0) {
      const props = new Map(input.props);
      for (const key of rest.excluded) props.delete(key.toString());
      return { input: { kind: "props", props }, pathOffset: 0 };
    }
    const nested = inputValueAt(input, rest.path);
    if (!nested) return { input, pathOffset: 0 };
    const states = nested.value.expressions.flatMap((expression) =>
      (nested.tail.length === 0
        ? [expression]
        : staticExpressionNodesAtPath(expression, nested.tail, new Set())
      ).flatMap((candidate) => objectPropVariants(candidate))
    );
    if (states.length === 0) return { input, pathOffset: 0 };
    const props = new Map<string, InputValue>();
    for (const state of states) {
      for (const [name, value] of state) {
        ...
        props.set(
          ...
          previous
            ? {
                expressions: [...previous.expressions, ...value.expressions],
                projections: [...previous.projections, ...value.projections],
                definitelyDefined:
                  previous.definitelyDefined === true &&
                  value.definitelyDefined === true,
                scanAsProp: previous.scanAsProp || value.scanAsProp,
              }
            : value
        );
      }
    ...
    for (const key of rest.excluded) props.delete(key.toString());
    return {
      input: { kind: "props", props },
      pathOffset: rest.path.length,
    };
  };
  function inputStaticVariants(
    ...
    const resolved = inputValueAt(input, path);
    if (!resolved) return [];
    const projected =
      resolved.tail.length === 0 ? resolved.value.projections : [];
    return uniqueVariants([
      ...projected,
      ...resolved.value.expressions.flatMap((expression) => {
        const variants = staticVariantsAtPath(expression, resolved.tail, seen);
        return variants.length > 0 ||
          resolved.tail.length > 0 ||
          !ts.isCallExpression(expression)
          ? variants
          : renderedCallVariants(expression);
      }),
    ]);
  }
  function inputDefinedAt(
    input: RenderInput,
    path: StaticPath,
    seen: ReadonlySet<StaticBindingDeclaration>
  ): boolean | null {
    ...
    if (
      resolved.tail.length === 0 &&
      resolved.value.definitelyDefined !== undefined
    ) {
      return resolved.value.definitelyDefined;
    }
    if (resolved.tail.length === 0 && resolved.value.projections.length > 0) {
      return true;
    }
    const states = resolved.value.expressions.map((expression) =>
      staticPathExists(expression, resolved.tail, seen)
    );
  ...
  const resolveReference: StaticReferenceResolver = (
    ...
    const boundInput = bindingRenderInput(binding, parameterInput(declaration));
    const input = boundInput?.input ?? null;
    const requestedPath = [...binding.path, ...path].slice(
      boundInput?.pathOffset ?? 0
    );
    const inputVariants = input
      ? inputStaticVariants(input, requestedPath, nextSeen)
      : [];
    if (inputVariants.length > 0) return inputVariants[0];
    if (input && inputDefinedAt(input, requestedPath, nextSeen) === true) {
      return null;
    }
    const resolved =
      !input && declaration.initializer
    ...
    for (const fallback of binding.defaults) {
      ...
      if (
        (activeBasePath.length === 0 && input
          ? inputDefinedAt(input, fallback.triggerPath, nextSeen)
          : activeExpression &&
  ...
  function staticVariantsAtPath(
    ...
    if (ts.isCallExpression(expression) && path.length > 0) {
      ...
      return uniqueVariants(
        objectPropVariants(expression).flatMap((props) => {
          const value = props.get(head);
          return value
            ? [
                ...(tail.length === 0 ? value.projections : []),
                ...value.expressions.flatMap((candidate) =>
                  staticVariantsAtPath(candidate, tail, seen)
                ),
              ]
            : [];
        })
      ...
      if (ts.isObjectLiteralExpression(expression)) {
        for (const property of [...expression.properties].reverse()) {
          ...
          if (
            ...
            return withGetterRead(property, [], () =>
              withThisInput(property, expressionInput(expression), () =>
                uniqueVariants(
  ...
  function resolveReferenceVariants(
    ...
    const inputVariants = input
      ? inputStaticVariants(input, requestedPath, nextSeen)
      : [];
    if (inputVariants.length > 0) return inputVariants;
    if (input && inputDefinedAt(input, requestedPath, nextSeen) === true) {
      return [];
    }
    const variants =
  ...
  function staticExpressionNodesAtPath(
    expression: ts.Expression,
    path: StaticPath,
    seen: ReadonlySet<StaticBindingDeclaration>
  ): ts.Expression[] {
    if (
      ...
      return staticExpressionNodesAtPath(expression.expression, path, seen);
    }
    if (ts.isConditionalExpression(expression)) {
      return [
        ...staticExpressionNodesAtPath(expression.whenTrue, path, seen),
        ...staticExpressionNodesAtPath(expression.whenFalse, path, seen),
      ];
    }
    if (
    ...
    if (path.length > 0) {
      ...
      if (ts.isArrayLiteralExpression(expression) && typeof head === "number") {
        const element = expression.elements[head];
        return element && !ts.isSpreadElement(element)
          ? staticExpressionNodesAtPath(element, tail, seen)
          : [];
      }
    }
    if (expression.kind === ts.SyntaxKind.ThisKeyword) {
    ...
    if (reference) {
      ...
      if (!binding || !declaration || seen.has(declaration)) return [];
      const nextSeen = new Set(seen).add(declaration);
      const boundInput = bindingRenderInput(
        binding,
        parameterInput(declaration)
      );
      const input = boundInput?.input ?? null;
      const requestedPath = [...binding.path, ...reference.path, ...path].slice(
        boundInput?.pathOffset ?? 0
      );
      if (input) {
        ...
        return resolved
          ? resolved.value.expressions.flatMap((candidate) =>
              staticExpressionNodesAtPath(candidate, resolved.tail, nextSeen)
            )
          : [];
      }
    ...
    return path.length === 0 ? [expression] : [];
  }
  ...
  type PropState = Map<string, InputValue>;
  const cloneProps = (props: ReadonlyMap<string, InputValue>): PropState =>
    new Map(props);
  const objectPropVariants = (
    expression: ts.Expression,
    seen: ReadonlySet<StaticBindingDeclaration> = new Set()
  ): PropState[] => {
    if (
      ...
      return objectPropVariants(expression.expression, seen);
    }
    if (ts.isObjectLiteralExpression(expression)) {
      let states: PropState[] = [new Map()];
      for (const property of expression.properties) {
        if (ts.isSpreadAssignment(property)) {
          const spreadStates = objectPropVariants(property.expression, seen);
          if (spreadStates.length > 0) {
            states = states.flatMap((state) =>
              spreadStates.map((spread) => {
                const merged = cloneProps(state);
                for (const [name, value] of spread) merged.set(name, value);
                return merged;
              })
            );
          }
    ...
    if (ts.isConditionalExpression(expression)) {
      return [
        ...objectPropVariants(expression.whenTrue, seen),
        ...objectPropVariants(expression.whenFalse, seen),
      ];
    }
    if (ts.isBinaryExpression(expression)) {
      if (
        ...
        return [new Map(), ...objectPropVariants(expression.right, seen)];
      }
      if (
        expression.operatorToken.kind === ts.SyntaxKind.BarBarToken ||
        expression.operatorToken.kind === ts.SyntaxKind.QuestionQuestionToken
      ) {
        const left = objectPropVariants(expression.left, seen);
        const right = objectPropVariants(expression.right, seen);
        return [
          ...(left.length > 0 ? left : [new Map()]),
          ...(right.length > 0 ? right : [new Map()]),
        ];
      }
    }
    if (ts.isCallExpression(expression)) {
    ...
    const boundInput = bindingRenderInput(binding, parameterInput(declaration));
    const input = boundInput?.input ?? null;
    const requestedPath = [...binding.path, ...reference.path].slice(
      boundInput?.pathOffset ?? 0
    );
    if (input) {
      if (input.kind === "props" && requestedPath.length === 0) {
        return [cloneProps(input.props)];
      }
      const resolved = inputValueAt(input, requestedPath);
      return resolved
        ? resolved.value.expressions.flatMap((candidate) =>
  ...
  const jsxPropVariants = (
    ...
    for (const property of attributes.properties) {
      if (ts.isJsxSpreadAttribute(property)) {
        const spreadStates = objectPropVariants(property.expression);
        if (spreadStates.length > 0) {
          states = states.flatMap((state) =>
            spreadStates.map((spread) => {
              const merged = cloneProps(state);
      ...
      if (!property.initializer) {
        ...
        value = {
          expressions: [property.initializer.expression],
          projections: [],
        };
      } else {
        continue;
    ...
    if (children && children.length > 0) {
      const projections = renderChildren(children);
      states = states.map((state) => {
        const next = cloneProps(state);
        next.set("children", {
          expressions: [],
  ...
  const collectPropCandidates = (
    ...
    for (const state of states) {
      for (const [name, value] of state) {
        if (value.scanAsProp === false) continue;
        if (!isCopyProp(name, customComponent)) continue;
        attributeCandidates.push(...value.projections);
        for (const expression of value.expressions) {
          attributeCandidates.push(...expressionVariants(expression));
        }
      }
    }
  };
  ...
  const renderElement = (
    ...
    if (ts.isJsxSelfClosingElement(node)) {
      ...
      if (components.length > 0) {
        ...
        return variants.length > 0 ? variants : [empty];
      }
      return [empty];
    }
  ...
  function inputJsxNodes(
    ...
    return resolved
      ? resolved.value.expressions.flatMap((expression) =>
          jsxNodesAtPath(expression, resolved.tail, seen)
        )
      : [];
  }
  ...
  function objectCallPropVariants(node: ts.CallExpression): PropState[] {
    return resolvedInvocations(node).flatMap((resolved) =>
      functionObjectPropVariants(
        resolved.node,
  ...
  ): { target: ts.Expression; name: string } | null => {
    if (ts.isPropertyAccessExpression(expression)) {
      return { target: expression.expression, name: expression.name.text };
    }
    if (
      ts.isElementAccessExpression(expression) &&
      expression.argumentExpression &&
  ...
  const activeCallableCalls = new Set<ts.FunctionLikeDeclaration>();
  function functionCallableVariants(
  ...
  const callableNodesAtPath = (
    ...
    if (
      ...
      return callableNodesAtPath(expression.expression, path, seen);
    }
    if (ts.isConditionalExpression(expression)) {
      return [
        ...callableNodesAtPath(expression.whenTrue, path, seen),
        ...callableNodesAtPath(expression.whenFalse, path, seen),
      ];
    }
    if (
      ts.isBinaryExpression(expression) &&
      ...
      return [
        ...callableNodesAtPath(expression.left, path, seen),
        ...callableNodesAtPath(expression.right, path, seen),
      ];
    }
    if (ts.isCallExpression(expression)) {
    ...
    if (
      ...
      const key = ts.isNumericLiteral(expression.argumentExpression)
        ? Number(expression.argumentExpression.text)
        : expression.argumentExpression.text;
      return callableNodesAtPath(expression.expression, [key, ...path], seen);
    }
    if (path.length > 0 && ts.isObjectLiteralExpression(expression)) {
      const [head, ...tail] = path;
      for (const property of [...expression.properties].reverse()) {
        if (
          ts.isPropertyAssignment(property) &&
          ...
          return callableNodesAtPath(property.initializer, tail, seen);
        }
        if (
          ts.isMethodDeclaration(property) &&
          propertyKey(property.name) === head
        ...
        if (
          ...
          const receiver = expressionInput(expression);
          const candidates = withGetterRead(property, [], () =>
            withThisInput(property, receiver, () =>
              functionReturnExpressions(property).flatMap((returned) =>
                callableNodesAtPath(returned, tail, seen)
              )
            )
          );
          for (const candidate of candidates) {
            if (ts.isArrowFunction(candidate.node)) {
              lexicalThisInputs.set(candidate.node, receiver);
            }
          }
          return candidates;
        }
        ...
        if (ts.isSpreadAssignment(property)) {
          const candidates = callableNodesAtPath(
            property.expression,
            path,
            seen
          );
    ...
    if (
      ...
      const [head, ...tail] = path;
      const element = expression.elements[head as number];
      return element && !ts.isSpreadElement(element)
    ...
    if (reference) {
      const direct =
        path.length === 0 && reference.path.length === 0
          ? resolveFunction(reference.identifier)
          : null;
      if (direct) return [{ node: direct, leadingInputs: [] }];
      const binding = lexicalBinding(reference.identifier);
      const declaration = binding?.declaration;
      if (!binding || !declaration || seen.has(declaration)) return [];
      const boundInput = bindingRenderInput(
      ...
      if (input) {
        ...
        return resolved
          ? resolved.value.expressions.flatMap((candidate) =>
              callableNodesAtPath(candidate, resolved.tail, nextSeen)
            )
          : [];
      }
      return declaration.initializer
        ? callableNodesAtPath(declaration.initializer, requestedPath, nextSeen)
        : [];
    }
  ...
  const markCallable = (resolved: ResolvedCallable): void => {
    if (resolved.leadingInputs.length === 0 && !resolved.receiver) {
      markFunction(resolved.node);
      return;
    }
    renderedRoots.push(
      ...functionRenderedVariants(
  ...
  const markExpression = (node: ts.Expression): void => {
    ...
    if (ts.isCallExpression(node)) {
      renderedRoots.push(...renderedCallVariants(node));
      return;
    }
  };
...
describe("copy-lint: user-facing tone standard (issue #945)", () => {
  ...
  }, 30_000); // weakening the scanner or raising the timeout for unrelated tests. // Coverage instrumentation measured 6.28s; leave CI headroom without
  ...
  it("applies defaults to statically undefined object and array values", () => {
    const banned = [
      [
        'const { copy = "Informational only." } = { copy: undefined };',
        "return <p>{copy}</p>;",
      ].join("\n"),
      [
        'const [copy = "Consult a clinician."] = [undefined];',
        "return <p>{copy}</p>;",
      ].join("\n"),
    ];
    ...
    const safe = [
      'const { copy = "Status" } = { copy: undefined };',
      "return <p>{copy}</p>;",
    ].join("\n");
    expect(disclaimerCopyViolations("synthetic.tsx", safe)).toEqual([]);
  });

  it("propagates rendered helper, map, component, and children inputs", () => {
    const renderedInputs = [
      [
        'function renderNotice(copy = "Status") { return <p>{copy}</p>; }',
        'return renderNotice("Informational only.");',
      ].join("\n"),
      [
        'const copies = ["Consult a clinician."];',
        'const renderNotice = (copy = "Status") => <p>{copy}</p>;',
        "return <main>{copies.map(renderNotice)}</main>;",
      ].join("\n"),
      [
        'function Notice({ message = "Status" }) { return <p>{message}</p>; }',
        'return <Notice message="Informational only." />;',
      ].join("\n"),
      [
        "function Notice({ children }) { return <p>{children}</p>; }",
        "return <Notice>Consult a clinician.</Notice>;",
      ].join("\n"),
    ];
    for (const sample of renderedInputs) {
      expect(
        disclaimerCopyViolations("synthetic.tsx", sample),
        sample
      ).toHaveLength(1);
    ...
    const overriddenDefaults = [
      [
        'function renderNotice(copy = "Informational only.") { return <p>{copy}</p>; }',
        'return renderNotice("Status");',
      ].join("\n"),
      [
        'const copies = ["Status"];',
        'const renderNotice = (copy = "Informational only.") => <p>{copy}</p>;',
        "return <main>{copies.map(renderNotice)}</main>;",
      ].join("\n"),
      [
        'function Notice({ message = "Informational only." }) { return <p>{message}</p>; }',
        'return <Notice message="Status" />;',
      ].join("\n"),
    ];
    for (const sample of overriddenDefaults) {
      expect(disclaimerCopyViolations("synthetic.tsx", sample), sample).toEqual(
        []
      );
    }
  ...
  it("resolves spread sources and applies JSX override precedence", () => {
    const detected = [
      [
        'const props = { message: "Informational only." } as const;',
        "return <Notice {...props} />;",
      ].join("\n"),
      [
        'const groups = { notice: { message: "Consult a clinician." } };',
        "return <Notice {...groups.notice} />;",
      ].join("\n"),
      'return <Notice {...(show && { message: "Informational only." })} />;',
      'return <Notice message="Status" {...{ message: "Informational only." }} />;',
    ];
    ...
    const overridden =
      'return <Notice {...{ message: "Informational only." }} message="Status" />;';
    expect(disclaimerCopyViolations("synthetic.tsx", overridden)).toEqual([]);
  });

  it("discovers indirect exports and rendered JSX member components", () => {
    const samples = [
      [
        "const Page = () => <p>Informational only.</p>;",
        "export { Page };",
      ].join("\n"),
      [
        "const Page = () => <p>Consult a clinician.</p>;",
        "export default Page;",
      ].join("\n"),
      [
        "const Parts = { Notice: () => <p>Informational only.</p> };",
        "export function Page() { return <Parts.Notice />; }",
      ].join("\n"),
      [
        "function Notice() { return <p>Consult a clinician.</p>; }",
        "const Parts = { Notice };",
        "export function Page() { return <Parts.Notice />; }",
      ].join("\n"),
    ];
  ...
  it("forwards computed copy through whole component props", () => {
    const rendered = [
      "const buildCopy = (lead, tail) => `${lead} ${tail}`;",
      'function Notice({ message = "Status" }) { return <p>{message}</p>; }',
      "function Wrapper(props) { return <Notice {...props} />; }",
      'return <Wrapper message={buildCopy("Informational", "only.")} />;',
    ].join("\n");
  ...
  it("reads static object properties through spreads in override order", () => {
    const detected = [
      [
        'const base = { message: "Consult a clinician." };',
        'const message = { message: "Status", ...base }.message;',
        "return <p>{message}</p>;",
      ].join("\n"),
      [
        'const message = { ...{ message: "Informational only." } }.message;',
        "return <p>{message}</p>;",
      ].join("\n"),
    ];
    for (const sample of detected) {
    ...
    const finalOverrides = [
      ...
      'const message = { ...base, message: "Status" }.message;',
      "return <p>{message}</p>;",
    ].join("\n");
    expect(disclaimerCopyViolations("synthetic.tsx", finalOverrides)).toEqual(
      []
    );
  });

  it("resolves callable object members in components, helpers, and map callbacks", () => {
    const samples = [
      [
        "const Parts = { Notice() { return <p>Informational only.</p>; } };",
        "return <Parts.Notice />;",
      ].join("\n"),
      [
        "const helpers = { notice() { return <p>Consult a clinician.</p>; } };",
        "return helpers.notice();",
      ].join("\n"),
      [
        'const copies = ["Informational only."];',
        "const renderers = { notice(copy) { return <p>{copy}</p>; } };",
        "return <main>{copies.map(renderers.notice)}</main>;",
      ].join("\n"),
    ];
    for (const sample of samples) {
      expect(
  ...
  it("preserves whole props through identity helpers and object rest bindings", () => {
    ...
    for (const identity of [
      ...
      expect(
        disclaimerCopyViolations("synthetic.tsx", wrappedRendered),
        wrappedRendered
      ).toHaveLength(1);
      ...
      expect(
        disclaimerCopyViolations("synthetic.tsx", wrappedSafe),
        wrappedSafe
      ).toEqual([]);
    }
    ...
    const nestedRestRendered = [
      'function Notice({ message = "Status" }) { return <p>{message}</p>; }',
      "function Wrapper({ config: { internal, ...props } }) {",
      "  return <Notice {...props} />;",
      "}",
      'return <Wrapper config={{ internal: "x", message: "Informational only." }} />;',
    ].join("\n");
    expect(
      disclaimerCopyViolations("synthetic.tsx", nestedRestRendered)
    ).toHaveLength(1);

    const nestedRestSafe = [
      'function Notice({ message = "Status" }) { return <p>{message}</p>; }',
      "function Wrapper({ config: { message, ...props } }) {",
      "  return <Notice {...props} />;",
      "}",
      'return <Wrapper config={{ message: "Informational only." }} />;',
    ].join("\n");
    expect(disclaimerCopyViolations("synthetic.tsx", nestedRestSafe)).toEqual(
      []
    );

    const deeplyNestedRestSafe = [
      'function Notice({ message = "Status" }) { return <p>{message}</p>; }',
      "function Wrapper({ config: { nested: { message, ...props } } }) {",
      "  return <Notice {...props} />;",
      "}",
      'return <Wrapper config={{ nested: { message: "Informational only." } }} />;',
    ].join("\n");
    expect(
      disclaimerCopyViolations("synthetic.tsx", deeplyNestedRestSafe)
    ).toEqual([]);

    const deeplyNestedRestRendered = [
      'function Notice({ message = "Status" }) { return <p>{message}</p>; }',
      "function Wrapper({ config: { nested: { internal, ...props } } }) {",
      "  return <Notice {...props} />;",
      "}",
      'return <Wrapper config={{ nested: { internal: "x", message: "Informational only." } }} />;',
    ].join("\n");
    expect(
      disclaimerCopyViolations("synthetic.tsx", deeplyNestedRestRendered)
    ).toHaveLength(1);
  });
  ...
  it("treats a later undefined spread property as an overriding value", () => {
    ...
    const helperOverride = [
      "const identity = (value) => value;",
      'const base = { message: "Informational only." };',
      "const message = { ...base, ...identity({ message: undefined }) }.message;",
      "return <p>{message}</p>;",
    ].join("\n");
    expect(disclaimerCopyViolations("synthetic.tsx", helperOverride)).toEqual(
      []
    );
    ...
    const helperBanned = [
      ...
      'const message = { ...base, ...identity({ message: "Informational only." }) }.message;',
      "return <p>{message}</p>;",
    ].join("\n");
    expect(
      disclaimerCopyViolations("synthetic.tsx", helperBanned)
    ).toHaveLength(1);
  });
  ...
  it("preserves receiver context for sibling calls and map thisArg", () => {
    ...
    const lexicalArrow = [
      "const helpers = {",
      '  prefix: "Informational ",',
      "  notice() { return (() => <p>{this.prefix}only.</p>)(); },",
      "};",
      "return helpers.notice();",
    ].join("\n");
    expect(
      disclaimerCopyViolations("synthetic.tsx", lexicalArrow)
    ).toHaveLength(1);

    const lexicalArrowSafe = lexicalArrow.replace(
      'prefix: "Informational "',
      'prefix: "Status: "'
    );
    expect(disclaimerCopyViolations("synthetic.tsx", lexicalArrowSafe)).toEqual(
      []
    );
  });

  it("models call, apply, and bind receivers and arguments", () => {
    const called = [
      [
        "function notice() { return <p>{this.message}</p>; }",
        'return notice.call({ message: "Informational only." });',
      ].join("\n"),
      [
        "function notice() { return <p>{this.message}</p>; }",
        'return notice.apply({ message: "Consult a clinician." }, []);',
      ].join("\n"),
      [
        "function notice(message) { return <p>{message}</p>; }",
        'return notice.call(null, "Informational only.");',
      ].join("\n"),
      [
        "function notice(message) { return <p>{message}</p>; }",
        'const args = ["Consult a clinician."];',
        "return notice.apply(null, args);",
      ].join("\n"),
      [
        "function Notice(message) { return <p>{this.prefix}{message}</p>; }",
        'const Bound = Notice.bind({ prefix: "Informational " }, "only.");',
        "export { Bound };",
      ].join("\n"),
    ];
    for (const sample of called) {
      expect(
        disclaimerCopyViolations("synthetic.tsx", sample),
        sample
      ).toHaveLength(1);
    ...
    for (const sample of called) {
      const safe = sample
        .replace("Informational only.", "Status")
        .replace("Consult a clinician.", "Status")
        .replace('prefix: "Informational "', 'prefix: "Status: "');
      expect(disclaimerCopyViolations("synthetic.tsx", safe), safe).toEqual([]);
    }

    const arrowKeepsLexicalThis = [
      "export function Page() {",
      "  const notice = () => <p>{this.prefix}only.</p>;",
      '  return notice.call({ prefix: "Informational " });',
      "}",
    ].join("\n");
    expect(
      disclaimerCopyViolations("synthetic.tsx", arrowKeepsLexicalThis)
    ).toEqual([]);
  });
  ...
  it("projects statically rendered getter values", () => {
    ...
    const receiver = [
      "const copy = {",
      '  prefix: "Consult a ",',
      '  get message() { return this.prefix + "clinician."; },',
      "};",
      "return <p>{copy.message}</p>;",
    ].join("\n");
    expect(disclaimerCopyViolations("synthetic.tsx", receiver)).toHaveLength(1);

    const receiverSafe = receiver.replace(
      'prefix: "Consult a "',
      'prefix: "Contact a "'
    );
    expect(disclaimerCopyViolations("synthetic.tsx", receiverSafe)).toEqual([]);

    const callable = [
      "const Parts = {",
      "  get Notice() { return () => <p>Informational only.</p>; },",
      "};",
      "return <Parts.Notice />;",
    ].join("\n");
    expect(disclaimerCopyViolations("synthetic.tsx", callable)).toHaveLength(1);

    const callableSafe = callable.replace("Informational only.", "Status");
    expect(disclaimerCopyViolations("synthetic.tsx", callableSafe)).toEqual([]);

    const callableReceiver = [
      "const Parts = {",
      '  prefix: "Informational ",',
      "  get Notice() { return () => <p>{this.prefix}only.</p>; },",
      "};",
      "return <Parts.Notice />;",
    ].join("\n");
    expect(
      disclaimerCopyViolations("synthetic.tsx", callableReceiver)
    ).toHaveLength(1);

    const callableReceiverSafe = callableReceiver.replace(
      'prefix: "Informational "',
      'prefix: "Status: "'
    );
    expect(
      disclaimerCopyViolations("synthetic.tsx", callableReceiverSafe)
    ).toEqual([]);
    ...
    expect(disclaimerCopyViolations("synthetic.tsx", composed)).toHaveLength(1);
    expect(
      disclaimerCopyViolations(
        "synthetic.tsx",
        composed.replace('prefix: "Informational "', 'prefix: "Status: "')
      )
    ).toEqual([]);

    const siblingCall = [
      "const copy = {",
      '  build() { return "Consult a clinician."; },',
    ...
    expect(
      disclaimerCopyViolations(
        "synthetic.tsx",
        siblingCall.replace("Consult a clinician.", "Status")
      )
    ).toEqual([]);

    const callableFactory = [
      "const Parts = {",
      "  make() { return () => <p>{this.prefix}only.</p>; },",
      '  prefix: "Informational ",',
      "  get Notice() { return this.make(); },",
      "};",
      "return <Parts.Notice />;",
    ].join("\n");
    expect(
      disclaimerCopyViolations("synthetic.tsx", callableFactory)
    ).toHaveLength(1);
    expect(
      disclaimerCopyViolations(
        "synthetic.tsx",
        callableFactory.replace(
          'prefix: "Informational "',
          'prefix: "Status: "'
        )
      )
    ...
    const callableCycle = [
      "const Parts = {",
      "  make() { return this.make(); },",
      "  get Notice() { return this.make(); },",
      "};",
      "return <Parts.Notice />;",
    ].join("\n");
    expect(disclaimerCopyViolations("synthetic.tsx", callableCycle)).toEqual(
      []
    );
