...
// THE WORKER'S MCP GRANT IS THE WRITE SET AND NOTHING ELSE
// (owner, 2026-08-30). environment.md §GitHub access has said "REST for every
// read" all along, but the work skill still GRANTED the MCP read tools
...
describe("work skill tool grants", () => {
  const text = fs.readFileSync(
    path.join(process.cwd(), ".claude/skills/work/SKILL.md"),
    "utf8"
  );
