...
describe("resolveSwitch", () => {
  it("names the direction the wall clock moved", () => {
    expect(resolveSwitch({ at: NOON_UTC, from: NY, to: TOKYO })?.direction).toBe(
      "forward"
    );
    expect(resolveSwitch({ at: NOON_UTC, from: TOKYO, to: NY })?.direction).toBe(
      "backward"
    );
  ...
  it("answers null for a record it cannot trust, instead of throwing", () => {
    expect(resolveSwitch({ at: "not-an-instant", from: NY, to: TOKYO })).toBeNull();
    expect(resolveSwitch({ at: NOON_UTC, from: "Mars/Olympus", to: TOKYO })).toBeNull();
    expect(resolveSwitch({ at: NOON_UTC, from: NY, to: "Mars/Olympus" })).toBeNull();
  });
});
...
describe("neverOccurred — the eastward vanished span", () => {
  ...
  it("covers the wall clock the switch jumped over", () => {
    ...
    expect(neverOccurred(flight, { day: "2026-05-01", minute: MIDDAY })).toBe(true);
    expect(neverOccurred(flight, { day: "2026-05-01", minute: EVENING })).toBe(true);
    expect(neverOccurred(flight, { day: "2026-05-01", minute: BEDTIME })).toBe(true);
  });

  it("leaves the wall clock BEFORE the jump alone — that morning happened", () => {
    expect(neverOccurred(flight, { day: "2026-05-01", minute: MORNING })).toBe(false);
  });
  ...
  it("is open at BOTH ends: the minute left and the minute landed on both happened", () => {
    ...
    expect(neverOccurred(flight, { day: "2026-05-01", minute: 10 * 60 })).toBe(false);
    expect(neverOccurred(flight, { day: "2026-05-01", minute: 23 * 60 })).toBe(false);
    expect(neverOccurred(flight, { day: "2026-05-01", minute: 10 * 60 + 1 })).toBe(true);
    expect(neverOccurred(flight, { day: "2026-05-01", minute: 23 * 60 - 1 })).toBe(true);
  });

  it("does not reach the next local day — the day after a switch is ordinary", () => {
    expect(neverOccurred(flight, { day: "2026-05-02", minute: MORNING })).toBe(false);
    expect(neverOccurred(flight, { day: "2026-04-30", minute: EVENING })).toBe(false);
  });

  it("spans the calendar day the switch skipped over the date line", () => {
    // Honolulu 2026-05-01 11:00 → Tokyo 2026-05-02 06:00.
    const dateLine: TimezoneSwitch = { at: EVENING_UTC, from: HONOLULU, to: TOKYO };
    expect(neverOccurred(dateLine, { day: "2026-05-01", minute: EVENING })).toBe(true);
    expect(neverOccurred(dateLine, { day: "2026-05-02", minute: 5 * 60 })).toBe(true);
    expect(neverOccurred(dateLine, { day: "2026-05-02", minute: MORNING })).toBe(false);
    expect(neverOccurred(dateLine, { day: "2026-05-01", minute: MORNING })).toBe(false);
  });

  it("never fires for a westward or a level switch", () => {
    const westward: TimezoneSwitch = { at: NOON_UTC, from: TOKYO, to: NY };
    expect(neverOccurred(westward, { day: "2026-05-01", minute: MIDDAY })).toBe(false);
    const level: TimezoneSwitch = {
      at: NOON_UTC,
      from: "Europe/Paris",
      to: "Europe/Berlin",
    };
    expect(neverOccurred(level, { day: "2026-05-01", minute: MIDDAY })).toBe(false);
  });
});
...
describe("occurredTwice — the westward repeated span", () => {
  ...
  it("covers the wall clock the profile lives through a second time", () => {
    ...
    expect(occurredTwice(flight, { day: "2026-05-01", minute: MIDDAY })).toBe(true);
    expect(occurredTwice(flight, { day: "2026-05-01", minute: EVENING })).toBe(true);
  });

  it("is closed at BOTH ends: the clock stands on one and runs back through the other", () => {
    expect(occurredTwice(flight, { day: "2026-05-01", minute: 10 * 60 })).toBe(true);
    expect(occurredTwice(flight, { day: "2026-05-01", minute: 23 * 60 })).toBe(true);
    expect(occurredTwice(flight, { day: "2026-05-01", minute: 10 * 60 - 1 })).toBe(false);
    expect(occurredTwice(flight, { day: "2026-05-01", minute: 23 * 60 + 1 })).toBe(false);
  });

  it("leaves the morning already taken before the jump outside the span", () => {
    expect(occurredTwice(flight, { day: "2026-05-01", minute: MORNING })).toBe(false);
  });

  it("spans backwards over the date line", () => {
    // Tokyo 2026-05-02 06:00 back to Honolulu 2026-05-01 11:00.
    const dateLine: TimezoneSwitch = { at: EVENING_UTC, from: TOKYO, to: HONOLULU };
    expect(occurredTwice(dateLine, { day: "2026-05-01", minute: EVENING })).toBe(true);
    expect(occurredTwice(dateLine, { day: "2026-05-02", minute: 5 * 60 })).toBe(true);
    expect(occurredTwice(dateLine, { day: "2026-05-02", minute: MORNING })).toBe(false);
  });

  it("never fires for an eastward switch", () => {
    const eastward: TimezoneSwitch = { at: NOON_UTC, from: NY, to: TOKYO };
    expect(occurredTwice(eastward, { day: "2026-05-01", minute: MIDDAY })).toBe(false);
  });
});
...
describe("stored switch history", () => {
  it("round-trips through the stored JSON", () => {
    ...
    expect(parseTimezoneSwitches(serializeTimezoneSwitches(list))).toEqual(list);
  });
  ...
  it("keeps the newest MAX_STORED_SWITCHES and no more", () => {
    ...
    for (let i = 0; i < MAX_STORED_SWITCHES + 5; i++) {
      history = appendTimezoneSwitch(
        history,
        { at: `2026-04-30T00:00:${String(i).padStart(2, "0")}Z`, from: NY, to: TOKYO },
        now
      );
    }
...
describe("travelPrompt", () => {
  ...
  it("says nothing for a profile that is not the login's own", () => {
    ...
    expect(travelPrompt({ ...base, ownProfile: false })).toEqual({ kind: "none" });
  });
  ...
  it("says nothing when the browser names no zone, or names one that is not real", () => {
    expect(travelPrompt({ ...base, deviceZone: null })).toEqual({ kind: "none" });
  ...
  it("suppresses the offer for the dismissed zone only", () => {
    expect(travelPrompt({ ...base, dismissedZone: TOKYO })).toEqual({ kind: "none" });
    // A NEW zone re-raises it — the dismissal answered one question, not the trip.
    expect(
      travelPrompt({ ...base, deviceZone: "Europe/Paris", dismissedZone: TOKYO })
  ...
  it("reports the return when the device is back on the recorded home zone", () => {
    expect(
      travelPrompt({ ...base, deviceZone: NY, profileZone: TOKYO, homeZone: NY })
  ...
  it("treats a home zone equal to the profile's own zone as stale, not a trip", () => {
    ...
    expect(
      travelPrompt({ ...base, deviceZone: TOKYO, profileZone: NY, homeZone: NY })
...
describe("copy", () => {
  it("names the place, not the IANA path", () => {
    ...
    expect(zonePlaceLabel("America/Argentina/Buenos_Aires")).toBe("Buenos Aires");
