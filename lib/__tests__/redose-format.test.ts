...
describe("redoseCardLabel", () => {
  ...
  it("at max wins over open", () => {
    expect(
      redoseCardLabel(status({ open: true, atMax: true, countInWindow: 4 }))
    ).toBe("Max reached · 4 of 4 in 24h");
  });

  it("keeps confirmed-max open-window copy unchanged (#4254)", () => {
    expect(redoseCardLabel(status({ open: true, countInWindow: 2 }))).toBe(
  ...
  it("keeps the window guidance with no confirmed daily max", () => {
    ...
    expect(noMax({ open: true, countInWindow: 1 })).toBe(
      "Redose OK — min interval passed · 1 in 24h · no 24h limit on record"
    );
  });
  ...
  it("never says 'Max reached' when no maximum was confirmed", () => {
    ...
    const label = redoseCardLabel(
      status({ open: true, maxDailyCount: null, countInWindow: 12 })
    );
    expect(label).toBe(
      "Redose OK — min interval passed · 12 in 24h · no 24h limit on record"
    );
  ...
  it("keeps the cross-item tail with no confirmed max", () => {
    expect(
      ...
      "Redose OK — min interval passed · 1 in 24h across 2 items · no 24h limit on record"
    );
  });
...
// confirmed max with no warning, and a family-fed counter read "1 in 24h" where
...
describe("prnQuickLogLabel (#1717)", () => {
  ...
  it("says Max reached at the confirmed daily max", () => {
    const label = prnQuickLogLabel({
      name: "Ibuprofen",
      dose: "200 mg",
      status: status({ countInWindow: 4, atMax: true }),
      countInWindow: 4,
      maxDailyCount: 4,
    });
  ...
  it("names the wait when the interval hasn't passed", () => {
    const label = prnQuickLogLabel({
      name: "Ibuprofen",
      dose: "200 mg",
      status: status({ open: false, countInWindow: 1, opensInHours: 2 }),
      countInWindow: 1,
      maxDailyCount: 4,
    });
  ...
  it("states the same missing-ceiling fact as the in-app card (#4254)", () => {
    ...
    expect(redoseCardLabel(noMax)).toContain("no 24h limit on record");
  });

  it("says nothing extra when nothing has been logged at all", () => {
    expect(
      prnQuickLogLabel({
        name: "Tylenol",
        status: null,
...
describe("prnOverMaxDetail (#1854)", () => {
  it("mg basis, family: milligrams stated, members named, ceiling in mg/day", () => {
    ...
    expect(d).toContain("2400 mg logged in the last 24h");
    expect(d).toContain("summed from your logged dose amounts");
    expect(d).toContain("across Ibuprofen + Ibuprofen 800 mg");
    expect(d).toContain("most conservative confirmed max of 1200 mg in 24h");
  ...
  it("mg lower bound reads 'At least' and counts the amount-less doses", () => {
    ...
    expect(d).toContain("At least 1600 mg logged in the last 24h");
  ...
  it("count basis states doses — never implying mg precision", () => {
    ...
    expect(d).toContain(
      "5 doses logged in the last 24h vs your confirmed max of 4 in 24h"
    );
...
// repeating "None · Redose OK" once per medication. The cases below are the
...
describe("medChipsStatusLine", () => {
  ...
  it.each([
    ...
    [
      "both windows open",
      [window({ countInWindow: 0 }), window({ countInWindow: 0 })],
      "Nothing given in 24h · both windows open",
    ],
    [
      "three open windows say all, not both",
      [window({}), window({}), window({})],
      "3 given in 24h · all windows open",
    ],
    [
      "a partial counts the open ones",
      [window({}), window({ open: false, countInWindow: 2 })],
      "3 given in 24h · 1 of 2 windows open",
    ],
    [
      "a lone closed window is singular",
      [window({ open: false })],
      "1 given in 24h · window not open yet",
    ],
    [
      ...
      [window({ atMax: true, countInWindow: 4 }), window({ countInWindow: 1 })],
      "5 given in 24h · 1 of 2 windows open · 1 at max",
    ],
