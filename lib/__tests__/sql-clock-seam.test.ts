...
const ALLOW: Record<string, { count: number; why: string }> = {
  ...
  "lib/auth.ts": {
    count: 18,
    why: "session create/slide/expire/reap — session TTLs, which lib/clock.ts's header explicitly bars the seam from owning. The six added by #3053 are the revocation tombstones: `revoked_sessions.revoked_at` is stamped when a session is deliberately ended and read back only as a duration against the same 90-day session ceiling, so its calendar day never meets a today()-derived value. Two of the six are the LIVENESS clause the set-shaped tombstones select on — the same `expires_at`/`created_at` comparison the session lookup makes, so that a session which had already lapsed is never recorded as one somebody revoked.",
  },
  ...
  "lib/offline/writes.ts": {
    count: 6,
    why: "the mood store's updated_at audit stamps + the replayed_keys retention DELETE. The three per-column past-day corrections and the two optional-rating clears (#1488, extended per rating column by #1408) each stamp updated_at from SQL's clock; the column can't be interpolated without making the SQL unreadable to the profile-scoping scanner, so one literal statement per column means one stamp per statement. An audit stamp records when the write happened; nothing compares its calendar DAY to a today()-derived value. The check-in upsert is TWO statements since #3416 (one replaces the day's row, one merges for a check-in composed without the day in view — see MoodWriteSight) and both bind sqlNow() instead, which is why splitting it took this entry DOWN from 7 rather than up to 8.",
  },
