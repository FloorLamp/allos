...
const ALLOW_SQL: { file: string; includes: string; why: string }[] = [
  ...
  {
    file: "lib/migrations/versions/168-illness-episode-day-window.ts",
    includes: "SELECT seq FROM sqlite_sequence WHERE name = 'illness_episodes'",
    why: "migration 168 (#2232) AUTOINCREMENT preservation: reads the table's sqlite_sequence high-water mark before the rebuild so episode ids can never recycle (the recently-resolved dismissal and stale-nudge ack sets rely on it) — schema bookkeeping, never profile rows; the rebuild's own INSERT…SELECT copies every row with its profile_id.",
  },
  {
    file: "lib/migrations/versions/168-illness-episode-day-window.ts",
    includes: "INSERT INTO sqlite_sequence (name, seq) VALUES ('illness_episodes', ?)",
    why: "migration 168 (#2232) AUTOINCREMENT preservation, restore half (empty-table case): sqlite_sequence carries no profile rows.",
  },
  {
    file: "lib/migrations/versions/168-illness-episode-day-window.ts",
    includes: "UPDATE sqlite_sequence SET seq = ? WHERE name = 'illness_episodes'",
    why: "migration 168 (#2232) AUTOINCREMENT preservation, restore half: bumps the rebuilt table's sqlite_sequence back to the pre-rebuild high-water mark — schema bookkeeping, never profile rows.",
  },
  {
