...
// THE DIGEST IS THE PM'S (owner, 2026-09-02). catchup-digest.sh was a worker's
...
// hold the structural facts: the worker's check-in no longer runs a digest,
...
const digest = read("scripts/work/pm-digest.sh");

describe("the PM digest", () => {
  it("is not run from the worker's check-in", () => {
