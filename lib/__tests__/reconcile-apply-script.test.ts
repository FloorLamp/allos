...
describe("reconcile-apply visibility contract", () => {
  it("an issue WITH a comment chain gets the edit announced as a comment", () => {
    const run = runApply({ "7": { body: BODY, comments: 3 } }, PATCH_PLAN, [
      "--apply",
    ]);
    expect(run.status).toBe(0);
    const writes = run.calls.filter((c) => c.method !== "GET");
    expect(writes.map((c) => c.method)).toEqual(["PATCH", "POST"]);
    expect(writes[1].url).toContain("/issues/7/comments");
    const note = JSON.parse(writes[1].body ?? "{}").body as string;
  ...
  it("a QUIET issue gets the body edit and no comment — a sweep must not spam", () => {
    const run = runApply({ "7": { body: BODY, comments: 0 } }, PATCH_PLAN, [
      "--apply",
    ]);
    expect(run.status).toBe(0);
    const writes = run.calls.filter((c) => c.method !== "GET");
  ...
  it("--notify marks an in-flight issue as having readers, comment chain or not", () => {
    const run = runApply({ "7": { body: BODY, comments: 0 } }, PATCH_PLAN, [
      "--apply",
      "--notify",
      "7,42",
