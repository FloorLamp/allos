...
// a few lines below the loader, and that second value was the `todayStr` handed to
// the add workspace and the med card — so a request crossing profile-local midnight
// between the two calls seeded a start date one day off the day the staleness gate
// had already used.
//
...
// age-as-of-scan.test.ts: the medications loader is read as TEXT and must not
// resolve a day of its own. If a future reader needs the day here, take it from the
// intake context — that is the point.
...
/** Source with line and block comments removed, so prose about today() is not a hit. */
export function stripComments(text: string): string {
  return text.replace(/\/\*[\s\S]*?\*\//g, "").replace(/\/\/[^\n]*/g, "");
}

const source = fs.readFileSync(path.join(REPO, MED_DATA), "utf8");
const code = stripComments(source);

describe("the medications loader takes its day from the intake context (#4609)", () => {
  it("strips comments before scanning", () => {
    expect(stripComments("a; // today(profileId)\nb;")).toBe("a; \nb;");
    expect(stripComments("a;\n/* today(profileId) */\nb;")).toBe("a;\n\nb;");
    expect(stripComments("const d = today(profileId);")).toContain("today(");
  });
  ...
  it("loadMedicationsData resolves no day of its own", () => {
    ...
    expect(
      lines.map(([n, line]) => `${MED_DATA}:${n} — ${line.trim()}`),
