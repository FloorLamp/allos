...
> = {
  ...
  schema_migrations: {
    columns: ["applied_at"],
    why: "the runner's applied-set ledger (name-keyed migrations, #119's follow-up) — BORN canonical: the table is created by the runner itself and lib/migrations/runner.ts is its only writer, bound to instantNow(). No column DEFAULT, so SQLite's bare clock can never leak in.",
  },
  stream_frontiers: {
  ...
  fasts: {
    ...
    why: "migration 20260816-fasts (#2756) — BORN canonical: the fasting lifecycle's claimed start and end. The table is new, so the claim cannot be false, and the entry is what binds its first writer (lib/fast-store.ts, reached only through lib/fast-write.ts) to utcInstant() instead of letting the call site pick a shape. Both columns are compared in SQL — the active-fast lookup, the range read the history and the annotation ride, and the overlap scan — so a bare value beside a canonical one would sort wrong while the query still looked right. No DEFAULT on either. `created_at` beside them stays bare and is NOT claimed here.",
  },
  notify_lifecycle: {
    columns: ["at"],
