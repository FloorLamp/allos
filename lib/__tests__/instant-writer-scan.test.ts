...
> = {
  ...
  glucose_trace: {
    columns: ["ts"],
    why: "migration 20260819-glucose-trace (#2810) — BORN canonical: the CGM trace's per-point event instant. The table is new, so the claim cannot be false, and the entry is what binds its only writer (lib/glucose-trace-db.ts) to lib/date.ts's utcMinute rather than letting the call site pick a shape. It is COMPARED in SQL on every read — the day reader windows it against the half-open UTC range lib/local-day-window.ts derives, and a bare-shaped value beside a canonical bound would sort wrong while the query still looked right, which is exactly the hazard hr_minutes.ts carried until migration 164. No column DEFAULT.",
  },
  schema_migrations: {
    columns: ["applied_at"],
