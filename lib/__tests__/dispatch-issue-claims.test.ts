...
describe("trailerSession / branchTrailerRefusal — whose work is on this branch", () => {
  ...
  const mergeCommit = "Merge remote-tracking branch 'origin/main' into a branch";
...
describe("branchOwnerVerdict — which reader answered, and what it cost", () => {
  ...
  it("refuses on the trailer when NO open PR heads the branch — the gap #5177 leaves", () => {
    const { refusal, notes } = branchOwnerVerdict(BRANCH, SELF,
  ...
  it.each([
    ...
    [{ absent: "every commit on it is already in origin/main" }, "no unlanded commit"],
  ])("%#: a fallback that could not answer WARNS, never refuses", (read, said) => {
    const { refusal, notes } = branchOwnerVerdict(BRANCH, SELF,
      reads({ prs: [] }, read));
    expect(refusal).toBeNull();
    expect(notes.join("\n")).toContain(said);
