...
import icd10 from "@/lib/datasets/data/icd10-common.json";
...
const OUT = path.join(REPO, "lib/datasets/data/icd10-common.json");
...
describe("icd10-common.json dataset", () => {
  ...
  it("declares ICD-10-CM as its code system", () => {
    expect(icd10.meta.system).toBe("ICD-10-CM");
  });

  it("carries a curated common subset (over a hundred, well under the full set)", () => {
    expect(icd10.entries.length).toBeGreaterThan(100);
    expect(icd10.entries.length).toBeLessThan(2000);
  });

  it("gives every entry a unique, well-shaped ICD-10-CM code and a display name", () => {
    const codes = new Set<string>();
    for (const c of icd10.entries) {
  ...
  it("is emitted sorted by code for a stable diff", () => {
    const codes = icd10.entries.map((c) => c.code);
