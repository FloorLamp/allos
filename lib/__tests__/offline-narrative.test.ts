...
describe("PR → Finding adapters", () => {
  ...
  it("maps cardio distance / speed / duration PRs", () => {
    ...
    expect(cardioPrToFinding(speed, "km").detail).toBe(
      "fastest Cycle at 30 km/h"
    );
    expect(cardioPrToFinding(duration, "km").detail).toBe(
      "longest Row at 45 min"
    );
    expect(cardioPrToFinding(distance, "mi").detail).toBe(
      "longest Run at 6.21 mi"
    );
    expect(cardioPrToFinding(speed, "mi").detail).toBe(
