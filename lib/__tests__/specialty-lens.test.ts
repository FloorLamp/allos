...
describe("specialtyLineForVisit (#2921)", () => {
  it.each([
    ...
    ["provider NUCC ophthalmology", { providerSpecialtyCode: "207W00000X" }, "vision"],
    ["provider NUCC dentistry", { providerSpecialtyCode: "122300000X" }, "dental"],
    ["provider NUCC dermatology", { providerSpecialtyCode: "207N00000X" }, "skin"],
    ["provider NUCC audiologist", { providerSpecialtyCode: "231H00000X" }, "hearing"],
    ...
    ["dentistry label", { providerSpecialty: "General Practice Dentistry" }, "dental"],
    ["orthodontics label", { providerSpecialty: "Orthodontics" }, "dental"],

    // ── The facility, as fallback (#1055) ──
    ["facility code when the clinician has none", { facilitySpecialtyCode: "207N00000X" }, "skin"],
    ["facility label when the clinician has none", { facilitySpecialty: "Audiology" }, "hearing"],
    [
      "the clinician outranks the facility",
      { providerSpecialtyCode: "207W00000X", facilitySpecialtyCode: "122300000X" },
      "vision",
    ],
    ...
    ["visit type code (CPT)", { code: "92014", text: "Office Visit" }, "vision"],
    ...
    // ── Refusals ──
    ["a generic office visit", { text: "Office Visit follow up Dr Reyes" }, null],
    ...
    ["an otolaryngology provider", { providerSpecialty: "Otolaryngology", providerSpecialtyCode: "207Y00000X" }, null],
...
describe("specialtyLineForCondition (#2921)", () => {
  it.each([
    ...
    ["strabismus", { name: "Strabismus", code: "H50.05", codeSystem: "ICD-10-CM" }, "vision"],
    ["conjunctivitis, dotless", { name: "Conjunctivitis", code: "H1033", codeSystem: "ICD-10-CM" }, "vision"],
    ["hearing loss", { name: "SNHL", code: "H90.3", codeSystem: "ICD-10-CM" }, "hearing"],
    ["dental caries", { name: "Caries", code: "K02.9", codeSystem: "ICD-10-CM" }, "dental"],
    ["atopic dermatitis", { name: "Eczema", code: "L20.9", codeSystem: "ICD-10-CM" }, "skin"],
    // Shape alone identifies ICD-10 when the importer stated no system.
    ["no stated system", { name: "Myopia", code: "H52.13", codeSystem: null }, "vision"],

    // ── Refusals ──
    // K09-K14 are jaw cysts, stomatitis and salivary glands — oral, not the
    // Dental pane's subject.
    ["stomatitis, past the dental block", { name: "Stomatitis", code: "K12.0", codeSystem: "ICD-10-CM" }, null],
    ["a code in another chapter", { name: "Type 2 diabetes", code: "E11.9", codeSystem: "ICD-10-CM" }, null],
    // A NAME is never enough: conditions have no curated synonym list behind them.
    ["an uncoded eye condition", { name: "Strabismus", code: null, codeSystem: null }, null],
    ["a SNOMED-coded condition", { name: "Cataract", code: "193570009", codeSystem: "SNOMED CT" }, null],
    ["a code from a vocabulary we do not read", { name: "Something", code: "H50.05", codeSystem: "ICD-9-CM" }, null],
  ] as [string, { name: string; code: string | null; codeSystem: string | null }, SpecialtyLine | null][])(
    "%s → %s",
    (_name, condition, expected) => {
