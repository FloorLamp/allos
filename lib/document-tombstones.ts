...
// THE WRITE CORE TAKES THE ID A WRITE GATE RETURNED: `profileId` on writeDocumentTombstone
...
// module still runs auth-blind and app/(app)/medical/document-actions.ts still owns the gate.
// A branded number is still a number, so the reads and clearDocumentTombstone take one
// unchanged — the clear is reached from lib/medical-pipeline.ts as well as from two actions,
// so it has no single gated caller to take a minted id from.
//
