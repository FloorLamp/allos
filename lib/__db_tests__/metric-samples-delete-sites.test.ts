...
//   backstop  no seam precedes the delete. Where the delete runs with foreign keys
//             ON, the declared action nulls the id and leaves `source_kind` standing
...
const SITES: Site[] = [
  ...
  },
  {
    ...
    reaches: true,
    frees: "backstop",
    why: "The same profile delete, sweeping the tables that carry a profile_id of their own — `care_plan_items` among them. No seam precedes it, and it runs under the same `foreign_keys = OFF`, so the declared action does not run here either; the profile's own follow-up rows are removed with the profile.",
  },
  {
