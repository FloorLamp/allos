...
// EVERY lane, not just from Now. Not demoted to "Show everything": absent. Three hours a
// night the app knew about an unlogged usual Evening and silently declined to say so, for
// exactly the population an Evening routine describes.
//
// This fixture stands at 22:30 local, inside those three hours, and asserts BOTH halves
// of the disagreement off one profile: the offer stands, and the timing the dashboard now
// gives it agrees. `mealTimeWindows` at the same minute is asserted `expired` in the same
// test, so the fixture names what shipped rather than only what should happen.
//
...
describe("the usual-routine offer's window and its placement window (#3265)", () => {
  it("stands at 22:30 and is PLACED at 22:30, where the meal windows had expired", () => {
    ...
    // 1. The offer is computed and it stands — the four DB reads the dashboard pays.
    ...
    const timing = localTimeWindow(slot.opensAt, slot.endsBefore - 1);
    expect(resolveDashboardTiming(timing, LATE_EVENING_MINUTE)).toEqual({
      kind: "active",
    });
    ...
    const anchors = [
      getNotifySchedule(p).supplementMinutes.Morning,
      getNotifySchedule(p).supplementMinutes.Midday,
      getNotifySchedule(p).supplementMinutes.Evening,
  ...
  it("runs to the last minute of the local day and no further", () => {
    const p = seedEveningHabit("EveningEdges");
    const slot = currentFoodSlotWindow(p);
    const timing = localTimeWindow(slot.opensAt, slot.endsBefore - 1);

    expect(resolveDashboardTiming(timing, 15 * 60)).toEqual({ kind: "active" });
    expect(resolveDashboardTiming(timing, 1439)).toEqual({ kind: "active" });
    // 10:59 is Morning's last minute — before this window opens, so the offer for THIS
    // window is still ahead rather than gone.
    expect(resolveDashboardTiming(timing, 15 * 60 - 1)).toEqual({
      kind: "future-today",
      opensAt: 900,
    });
  });

  // THE HOURS WHERE THE DEFECT IS INVISIBLE, stated so the fixture above is not read as
  // a claim that the two windows never agreed. Inside a meal window they say the same
  ...
  it("agrees with the meal windows at 20:00, inside the Evening reminder window", () => {
    ...
    const slot = currentFoodSlotWindow(p);
    const timing = localTimeWindow(slot.opensAt, slot.endsBefore - 1);
    const anchors = [8 * 60, 13 * 60, 20 * 60];
    const atEight = 20 * 60;
    expect(resolveDashboardTiming(timing, atEight)).toEqual({ kind: "active" });
    expect(resolveDashboardTiming(mealTimeWindows(anchors), atEight)).toEqual({
      kind: "active",
    });
  });
});
