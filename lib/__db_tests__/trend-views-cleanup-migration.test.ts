// DB INTEGRATION TIER — migration 140 (issue #1869 item 4): the one-shot sweep of
...
//
// Built like the 113 fold test: a genuine pre-140 database (migrations 1…139 on a
// fresh in-memory handle), fixture rows in the old shape, run 140's up(), assert
...
import { up as up140 } from "@/lib/migrations/versions/140-trend-views-cleanup";

process.env.ADMIN_PASSWORD = process.env.ADMIN_PASSWORD ?? "db-test-admin-pw";

const CLEANUP_MIGRATION = 140;

// A database at exactly user_version 139 — every migration BEFORE the sweep. FKs
...
describe("migration 140 — trend_views cleanup (#1869)", () => {
