...
describe("a DEXA regional label is never offered as a Coverage candidate (#2319)", () => {
  it("declines the whole regional family while a genuine gap stays offered", () => {
    ...
    // actually sees it on (#2765, #2768). The spine density and ratio have their
    // own reasons; the remaining rows retain the shared decomposition reason.
    ...
    const ANDROID_GYNOID = "Android/Gynoid Ratio";
    for (const name of DEXA_REGIONAL.filter(
      (n) => n !== SPINE_BMD && n !== ANDROID_GYNOID
    ))
    ...
    expect(reasonFor(ANDROID_GYNOID)).toContain(
      "arithmetic over two regions of one DEXA scan"
    );
    expect(reasonFor(ANDROID_GYNOID)).not.toContain("population reference");
