...
// reading with `ORDER BY ended_at DESC LIMIT 1`. ended_at is NOT the third column
// of idx_metric_samples_md (profile_id, metric, date) — `date` derives from
// started_at in the profile timezone (#94) and can diverge from ended_at at day
// boundaries, so the sort key stays ended_at. Before the fix the planner served
...
// (profile_id, metric, ended_at) makes the ORDER BY an index-ordered reverse seek.
//
...
describe("getLatestMetricSample: ended_at ORDER BY is index-served (#115)", () => {
  it("plan uses idx_metric_samples_end with no temp b-tree sort", () => {
    ...
    // (profile_id, metric) slice to answer ORDER BY ended_at DESC LIMIT 1.
    expect(joined).not.toMatch(/TEMP B-TREE/i);
    // And it does so by walking the companion index in ended_at order.
    expect(joined).toMatch(/idx_metric_samples_end/);
  });

  it("still returns the reading with the newest ended_at", () => {
    // Day 28 has the latest ended_at; its value (1028) is the answer.
