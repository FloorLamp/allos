// DB INTEGRATION TIER — the server-side Training Log paging query (issue #451).
//
// getTrainingLogPage windows the feed by WHOLE DAYS via keyset ("seek") pagination on
...
import { getTrainingLogPage, getActivities } from "@/lib/queries";
...
beforeAll(() => {
  profileId = newProfile("training-log-page");
  otherProfile = newProfile("training-log-page-other");
...
describe("getTrainingLogPage — bounded, day-keyed windows (#451)", () => {
  it("returns at most dayLimit DISTINCT days and every activity on them", () => {
    const page = getTrainingLogPage(profileId, null, 14);
  ...
  it("nextBefore is the oldest loaded date while days remain, else null", () => {
    const first = getTrainingLogPage(profileId, null, 14);
    ...
    const all = getTrainingLogPage(profileId, null, 100);
  ...
  it("walking pages with the cursor equals the full unbounded feed, once each", () => {
    ...
    let guard = 0;
    for (;;) {
      const page = getTrainingLogPage(profileId, before, 7);
  ...
  it("returns an empty page (no cursor) for a profile with no activities", () => {
    const empty = newProfile("training-log-page-empty");
    const page = getTrainingLogPage(empty, null, 14);
