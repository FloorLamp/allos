...
// `occurred_at` the administration instant. The same questions now have the same names
// on both logging ledgers.
//
...
// order. Intake no longer needs one after #2876. A chain is legitimate only when every
// link answers the SAME question. `event` never
...
//
// Falling from `occurred_at` to `recorded_at` crosses questions, and that is the fall
