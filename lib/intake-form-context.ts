...
// SUBJECT of the write, gathered once, so a door cannot open with half of it.
//
// The form was fed by whichever host happened to mount it, and the hosts disagreed:
// /medications passed the full set while the illness add fold passed the pediatric
// context alone. Same form, same profile, different safety surface — a child got
// chronic-alcohol counselling because the food-note age gate ran on "unknown", and the
// stack-interaction, PGx and pairing controls rendered with nothing in them. Nothing
// marked the illness door as degraded; it looked complete.
//
// KEYED ON THE SUBJECT, NOT ON WHO IS LOOKING. The parameter is the profile the item
// is being recorded FOR. Every read below is that profile's, so a caregiver opening
// this door for a child gets the CHILD's age, stack, variants and conditions — not
// their own. Authorization stays at the request boundary: a caller resolves which
// profile it may write and passes that id, exactly as the rest of lib/ works.
...
export interface IntakeFormContext {
  ...
  // Every recorded condition with its status (#3650) — the indication and purpose
  // pickers offer the active ones and must still be able to name a resolved one.
  conditions: IntakeConditionOption[];
  // Age in months, latest weight, and the profile-local day. The form's ONE source of
  // the subject's age: the weight-band picker and the food-note age gate both read it,
  // so a door that can render pediatric dosing cannot also gate on an unknown age.
