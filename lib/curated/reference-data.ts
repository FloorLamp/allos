...
export const CURATED_LABS: Biomarker[] = [
  ...
  // (https://glaucoma.org/articles/what-is-considered-normal-eye-pressure).
  {
    ...
    optimal_high: null,
    direction: "in_range",
    note: "Fluid pressure inside the eye (tonometry), eye unspecified. Normal ~10–21 mmHg; above 21 is ocular hypertension, a glaucoma risk factor rather than glaucoma itself. AAO / Glaucoma Research Foundation.",
  },
  {
  ...
  // A DIAGNOSIS — a screening instrument, not a diagnostic. Public domain (PHQ/GAD).
  {
    ...
    optimal_high: null,
    direction: "lower_better",
    note: "Patient Health Questionnaire-9 total score (0–27), a validated screening instrument for depression severity. Severity bands: 0–4 minimal, 5–9 mild, 10–14 moderate, 15–19 moderately severe, 20–27 severe. Screening only. Item 9 asks about thoughts of self-harm. Public domain (Spitzer, Williams, Kroenke / Pfizer).",
  },
  {
    name: "GAD-7",
    category: "instrument",
    ...
    optimal_high: null,
    direction: "lower_better",
    note: "Generalized Anxiety Disorder-7 total score (0–21), a validated screening instrument for anxiety severity. Severity bands: 0–4 minimal, 5–9 mild, 10–14 moderate, 15–21 severe. Screening only. Public domain (Spitzer, Kroenke, Williams, Löwe).",
  },
  ...
  // item text is NOT reproduced anywhere in the app (see lib/substance-use.ts).
  {
    ...
    optimal_high: null,
    direction: "lower_better",
    note: "AUDIT-C total score (0–12), the 3-item alcohol-use screen (consumption items of the WHO AUDIT). Bands (UK PHE/NHS scoring): 0–4 lower risk, 5–7 increasing risk, 8–10 higher risk, 11–12 possible dependence; a score of 3+ (women) or 4+ (men) is commonly treated as a positive screen. Screening only. Public domain (Bush et al. 1998, VA).",
  },
  {
    name: "AUDIT",
    category: "instrument",
    ...
    optimal_high: null,
    direction: "lower_better",
    note: "AUDIT total score (0–40), the WHO 10-item Alcohol Use Disorders Identification Test. Risk zones (WHO manual): 0–7 lower risk, 8–15 increasing risk, 16–19 higher risk, 20–40 possible dependence. Screening only. Recorded as an outside-administered total (item text not reproduced in-app).",
  },
  {
    name: "DAST-10",
    category: "instrument",
    ...
    optimal_high: null,
    direction: "lower_better",
    note: "Drug Abuse Screening Test (DAST-10) total score (0–10) for non-alcohol drug use in the past 12 months. Bands (Skinner): 0 none reported, 1–2 low, 3–5 moderate, 6–8 substantial, 9–10 severe. Screening only. Recorded as an outside-administered total (item text not reproduced in-app; © H. A. Skinner / CAMH).",
  },
  ...
  // analyte; fetal fraction carries a % unit for display but never flags (`qc` class).
  {
    ...
    optimal_high: null,
    direction: "in_range",
    note: "Prenatal cell-free-DNA screen for trisomy 21 (Down syndrome). A low-/high-risk screen — a high-risk result is confirmed by diagnostic testing. No numeric band.",
  },
  {
    ...
    optimal_high: null,
    direction: "in_range",
    note: "Prenatal cell-free-DNA screen for trisomy 18 (Edwards syndrome). A low-/high-risk screen. No numeric band.",
  },
  {
    ...
    optimal_high: null,
    direction: "in_range",
    note: "Prenatal cell-free-DNA screen for trisomy 13 (Patau syndrome). A low-/high-risk screen. No numeric band.",
  },
  {
    name: "Fetal Fraction",
    category: "lab",
  ...
  },
  {
    ...
    optimal_high: null,
    direction: "higher_better",
    note: "The proportion of your forced vital capacity blown out in the first second, as a percentage. The one spirometry value with a universal cutoff: a post-bronchodilator ratio under 70% is the fixed criterion for persistent airflow obstruction, whatever your height or age. Read in clinical context. GOLD / ATS-ERS.",
  },
  // ── The #2300 unresolved-analyte close-out ──────────────────────────────────
  //
  // Every remaining distinct name a re-extraction of the older lab PDFs left
  // unresolved, minus the five deliberate exclusions (the three race-branched eGFR
  // variants and two toxicology screens). None of these is a new KIND of thing: the
  // urinalysis microscopy lines join the physical/dipstick family already here, the
  // CBC smear lines join the differential, and the three ratios are the analytes the
  // read-time derivation (lib/derived-biomarkers.ts) needed a row to land on.
  //
  // ALL RANGELESS, on purpose. Most are qualitative in the same way the dipstick pads
  // above are — a color, a clarity, a crystal type, a Negative/Positive screen — and
  // the three ratios plus the two smear percentages are quantities whose normal band
  // is a clinical judgement this dataset does not ship: albuminuria staging needs
  // repeat samples over months, and a band here would flag a single spot sample as
  // abnormal on its own. They still gain identity, a panel, a description and a
  // trendable series, which is what "unresolved" was costing them.
  //
  // Two of them are read-only by design: `Bilirubin, Indirect` is recorded when a lab
  // prints it and NEVER computed (a censored component makes the subtraction
  // undefined — labs print "Can't Calc"), and `Omega-6 Total` is never summed from the
  // itemized omega-6 lines (that sum omits DGLA and the minor species, so it
  // understates the printed total).
  {
    name: "Urine Color",
    category: "lab",
    unit: null,
    ...
    optimal_high: null,
    direction: "in_range",
    note: "Color of the urine specimen on the physical exam of a urinalysis (straw, yellow, amber, red). Qualitative — it shifts with hydration, diet and medications, so there is no normal band.",
  },
  {
    ...
    optimal_high: null,
    direction: "in_range",
    note: "Clarity of the urine specimen (clear, hazy, cloudy). Qualitative; cloudiness can come from cells, crystals, mucus or a contaminated collection.",
  },
  {
    name: "Bacteria, Urine",
    category: "lab",
    ...
    optimal_high: null,
    direction: "in_range",
    note: "Bacteria seen on urine microscopy. Usually reported semi-quantitatively (none / few / moderate / many) per high-power field; bacteria can mean infection or simply a contaminated collection, so it is read together with the white-cell count and nitrite.",
  },
  {
    name: "Casts, Hyaline, Urine",
    category: "lab",
    unit: "/LPF",
    ref_low: null,
    ref_high: null,
    ...
    optimal_high: null,
    direction: "in_range",
    note: "Hyaline casts on urine microscopy, per low-power field. The most common cast type and the least specific — a few are normal after exercise, dehydration or diuretics.",
  },
  {
    name: "Casts, Granular, Urine",
    category: "lab",
    unit: "/LPF",
    ref_low: null,
    ref_high: null,
    ...
    optimal_high: null,
    direction: "in_range",
    note: "Granular casts on urine microscopy, per low-power field. A few can follow exercise or dehydration; persistent numbers suggest tubular injury.",
  },
  {
    name: "Casts, RBC, Urine",
    category: "lab",
    unit: "/LPF",
    ref_low: null,
    ref_high: null,
    ...
    optimal_high: null,
    direction: "in_range",
    note: "Red-blood-cell casts on urine microscopy, per low-power field. They point to bleeding from the glomerulus rather than the lower urinary tract, so any are worth attention.",
  },
  {
    name: "Crystals, Urine",
    category: "lab",
    ...
    optimal_high: null,
    direction: "in_range",
    note: "Type of crystal seen on urine microscopy (calcium oxalate, uric acid, triple phosphate, …). Qualitative — several kinds occur in normal urine and depend on the specimen's pH and how long it sat.",
  },
  {
    name: "Crystal Amount, Urine",
    category: "lab",
    unit: null,
    ref_low: null,
    ref_high: null,
    ...
    optimal_high: null,
    direction: "in_range",
    note: "How many crystals were seen on urine microscopy (few / moderate / many), reported next to the crystal type. Qualitative.",
  },
  {
    ...
    optimal_high: null,
    direction: "in_range",
    note: "Urine albumin normalized to urine creatinine (ACR) — the standard way to read a spot urine albumin, because concentration alone tracks how dilute the sample is. KDIGO reads under 30 mg/g as normal and 30–300 as moderately increased albuminuria, but staging needs repeat samples over months, so no flagging band is set here. Uses URINE creatinine — never the serum value.",
  },
  {
    ...
    optimal_high: null,
    direction: "in_range",
    note: "Urine total protein normalized to urine creatinine — a spot-sample stand-in for a 24-hour protein collection. Uses URINE creatinine, never the serum value; interpretation depends on the clinical context, so no band is set here.",
  },
  {
    ...
    optimal_high: null,
    direction: "in_range",
    note: "Screen for blood in the stool (guaiac or fecal immunochemical test). Qualitative — Negative is normal; a positive is a reason to follow up, and the test does not replace a colonoscopy.",
  },
  {
    name: "Stool Color",
    category: "lab",
    ...
    optimal_high: null,
    direction: "in_range",
    note: "Color of the stool specimen (brown, green, black, red, clay). Qualitative and strongly diet- and medication-dependent.",
  },
  {
    ...
    optimal_high: null,
    direction: "in_range",
    note: "Consistency of the stool specimen (formed, soft, loose, watery). Qualitative.",
  },
  {
    ...
    optimal_high: null,
    direction: "in_range",
    note: "Microscopic examination of stool for parasite eggs (ova) and parasites. Qualitative — None seen is the normal result.",
  },
  {
    name: "Stool Red Blood Cells",
    category: "lab",
    unit: "cell/HPF",
    ref_low: null,
    ref_high: null,
    ...
    optimal_high: null,
    direction: "in_range",
    note: "Red blood cells seen on stool microscopy, per high-power field.",
  },
  {
    name: "Stool White Blood Cells",
    category: "lab",
    unit: "cell/HPF",
    ref_low: null,
    ref_high: null,
  ...
  },
  {
    ...
    optimal_high: null,
    direction: "in_range",
    note: "Reactive (atypical) lymphocytes on the blood smear, as a percentage of white cells. Reported ALONGSIDE the total lymphocyte fraction — a separate value, not a restatement of it — and commonly seen with viral infections.",
  },
  {
    ...
    optimal_high: null,
    direction: "in_range",
    note: "Immature (band) neutrophils on the blood smear, as a percentage of white cells. Reported ALONGSIDE the segmented neutrophil fraction — a separate value, not a restatement of it; a rise is the classic left shift of acute infection.",
  },
  {
    name: "Red Blood Cell Morphology",
    category: "lab",
    unit: null,
    ref_low: null,
    ref_high: null,
    ...
    optimal_high: null,
    direction: "in_range",
    note: "The smear reviewer's description of red-cell size, shape and color (normocytic normochromic, anisocytosis, poikilocytosis, …). Qualitative free text that qualifies the numeric red-cell indices.",
  },
  {
    ...
    optimal_high: null,
    direction: "in_range",
    note: "Unconjugated (indirect) bilirubin — total minus direct bilirubin. Recorded only when the lab prints it: when either component is reported below its detection limit the subtraction is undefined and labs print Can't Calc rather than guess, which is why this is not computed here.",
  },
  {
    ...
    optimal_high: null,
    direction: "in_range",
    note: "HDL cholesterol expressed as a percentage of total cholesterol, printed by some lipid panels. A higher share is generally more favorable, but it is read with the absolute values rather than against a band of its own.",
  },
  {
    name: "Omega-6 Total",
    category: "lab",
