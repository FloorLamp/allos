...
// source deliver continuously" has one answer for the detector, the reminder, and
// whatever asks next.
//
// Why the facet exists at all. `silenceToleranceMinutes` asks about the CONNECTION —
// has any successful run landed lately. A source can pass that while one of its
...
//
// A source that declares no stream is EXEMPT BY CONSTRUCTION. There is no exemption
...
// A stream together with the source that delivers it. Every reader below returns
// this rather than a bare stream, because a stream id is only unique WITHIN a
// source and every consumer needs both halves anyway.
...
/** The source's declared continuous streams — empty for one that declares none. */
 ...
 * paired with its source, in registry order.
 *
 ...
 */
export function allContinuousStreams(): SourceStream[] {
...
/** One source's stream by id, or null. */
 ...
 * its source and stream through this instead of naming `health-connect` and
...
export function streamsWithReminder(
  ...
  return allContinuousStreams().filter(
    (s): s is SourceStreamWithReminder => s.stream.reminder?.id === reminder
  );
}

/** A stream known to carry a reminder facet — see `streamsWithReminder`. */
export interface SourceStreamWithReminder extends SourceStream {
