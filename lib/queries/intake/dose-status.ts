...
// intake_items JOIN.
//
// SCHEDULED-DOSE STATUS: the reads that say where a dose stands, and THE ONE core
// (#2039) that moves it. Every transition of a scheduled dose's daily log row —
// taken, skipped, cleared — goes through `applyDoseStatusCore`, under one BEGIN
// IMMEDIATE, with its own supply crossing and a typed refusal. The four doors above
// it (the two one-way resolvers, the tri-state set, and the act→undo clear) differ
// only in what their caller is allowed to STATE; none of them writes the table itself.
//
// The reads sit here rather than in dose-history.ts because they answer about STATE,
// not about history: which items were taken today, which doses are taken or skipped
// on a date, what the strip shows over a window, and when each dose was first logged.
// The history module answers what was taken and prints it; these answer whether a
// dose is still owed, which is the question the core refuses or accepts on.
