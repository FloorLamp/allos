...
// fresh profile). SLOT-AWARE (issue #950): with a `window`, each food_log_events tap whose
// DERIVED slot matches feeds a second, slot-specific frecency signal that LEADS the blend
// with overall frecency backfilling; omitting `window` (or a cold slot) collapses to the
// pre-#950 overall order — no cliff. Then ONE demotion: the profile's EXCLUDED groups
...
// reads (#221). The slot signal derives each event's window at read time from
