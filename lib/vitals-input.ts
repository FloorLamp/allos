...
export function sleepWindowFromClocks(
  ...
  const wakeFromNoon = wakeMin + 720;
  return {
    bed,
    wake,
    bedOnPreviousDay,
    minutes: wakeFromNoon - bedFromNoon,
    durationStated,
  };
}
...
export const VITAL_CANONICAL = {
  ...
  // (see scripts/gen-canonical-result-definitions.ts CURATED_LABS) whose age/sex percentile
...
export function normalizeVitalsInput(
  ...
  const window = sleepWindowFromClocks(
    input.bedTime,
    input.wakeTime,
    sleepHours != null
  );
