...
// The findings-bus namespace for food suggestions (issue #435/#482). Keyed on the
// nutrient, so a dismiss follows the nutrient family, not a single biomarker reading.
...
// optimal range — the only direction diet can address by ADDING a food. A high reading
// never triggers a "eat more" suggestion.
...
export type FoodSafetyNoteKind = "allergy" | "medication" | "condition";
...
export interface FoodSuggestion {
  // Nutrient key (the dedupe family).
  key: string;
  label: string;
  // `food-suggest:<key>` — the findings-bus dedupeKey (family-keyed, #482).
  dedupeKey: string;
  // The flagged biomarker names that triggered this suggestion (for the "because your
  // … is low" rationale). Current, family-collapsed readings only.
  triggeredBy: string[];
  foods: SuggestedFood[];
  evidence: string;
  source: string;
...
export function suggestFoods(input: FoodSuggestInput): FoodSuggestion[] {
  // Index flagged readings by lowercased name for O(1) family lookup.
  ...
  for (const entry of ENTRIES) {
    ...
    const suggestion = buildSuggestion(entry, triggeredBy, input, drugHits);
...
export function nutrientFoodMapBiomarkers(): string[] {
  return [...new Set(ENTRIES.flatMap((e) => e.biomarkers))];
}
...
export { ENTRIES as NUTRIENT_FOOD_ENTRIES };
