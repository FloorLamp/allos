...
# container, never a worker's; a --peek or --since run leaves it alone, and a
# run with a failed read never advances it. Reads are unauthenticated REST per
# docs/work/environment.md §GitHub access plus git on origin/main.
#
# Usage:
#   bash scripts/work/pm-digest.sh              # since the last digest, then advance the anchor
#   bash scripts/work/pm-digest.sh --peek       # same window, anchor untouched
#   bash scripts/work/pm-digest.sh --since ISO  # explicit window start (implies --peek)
#   bash scripts/work/pm-digest.sh --days N     # last N days (implies --peek)
#
