#!/usr/bin/env bash
# Catch-up digest for an work check-in (see docs/work.md).
#
# Not the flight recorder — that is scripts/work-checkin.sh, which
...
# stays with the worker; this script only makes the gathering cheap and
...
# the recorder: a container reclaim wipes the worker's memory of when it
...
# docs/work/environment.md §GitHub access — no token needed, and the
# search endpoint is never used.
#
# Usage:
#   bash scripts/work/catchup-digest.sh              # digest, then advance the anchor
#   bash scripts/work/catchup-digest.sh --peek       # digest, leave the anchor alone
#   bash scripts/work/catchup-digest.sh --since ISO  # explicit window (implies --peek)
#
...
# Same resolver as work-checkin.sh and dispatch-brief.mjs (#3710).
...
if [ "${CATCHUP_SKIP_RECORDER:-0}" != "1" ]; then
  bash "$REPO_DIR/scripts/work-checkin.sh"
fi
...
PROC_PATHS='^(docs/work|scripts/work/|scripts/work-checkin\.sh|\.claude/|AGENTS\.md|CLAUDE\.md|docs/internals/e2e|\.github/workflows/)'
...
# hours. Whether any of it is a fire is the worker's call, and calling a
...
#     where the worker already looks. The gather script owns the
#     arithmetic; it needs a read token, and its absence is said, not skipped.
echo "--- release notes ---"
node "$REPO_DIR/scripts/work/release-notes-gather.mjs" --check 2>/dev/null ||
...
CORE="$REPO_DIR/scripts/work/merge-gate-core.mjs" node --input-type=module -e '
