...
// its own claims, by the same worker that wrote the brief, from the same
...
//
// Usage:
//   node scripts/work/adversarial-review-brief.mjs <pr-number> [--check]
//
...
//                    safety-relevant behaviour. The WORKER decides.
...
export const HIGH_STAKES = [
  ...
  // because no declared path matched, and the worker had to override the
...
// `matched: warning` sends the worker to go read the PR, which is the exact
...
function main(argv) {
  ...
  const surface = pathHits.length
    ...
    : vocabHits.length
      ...
      : "- (dispatched by --force; the surface is the worker's judgement)";
...
- Per claim: CONFIRMED (with what you ran) or REFUTED (with the exact failing
  input/sequence and its output, reproducible by the worker).
