...
//
// The work runbook's dispatch template was pasted into agent briefs
...
// worker session that created it (container restarts are the dominant
// failure mode — docs/work.md).
//
// Usage:
//   node scripts/work/dispatch-brief.mjs new --branch <branch> \
//     [--worktree wt-<name>] [--issues 123,456] [--task "one line"] \
//     [--e2e] [--port-base N] [--candidate] [--priority P1] \
//     [--lane user-data]
//   node scripts/work/dispatch-brief.mjs list
//   node scripts/work/dispatch-brief.mjs brief <branch>
//   node scripts/work/dispatch-brief.mjs promote <branch>
//   node scripts/work/dispatch-brief.mjs update <branch> \
//     [--priority P1] [--lane user-data]
//   node scripts/work/dispatch-brief.mjs done <branch> [--keep]
//   node scripts/work/dispatch-brief.mjs resume <branch>
//   node scripts/work/dispatch-brief.mjs adopt <branch> \
//     [--issues 123,456] [--task "one line"] [--e2e] [--port-base N]
//   node scripts/work/dispatch-brief.mjs claims <path>
//
...
//   worker that lost it (restart, compaction, a truncated tail) used to
...
//   worker — was incomplete by construction. One path, not two.
//
...
// worker, and dispatch failed at worktree setup).
...
// $SCRATCH/.roster is what scripts/work-checkin.sh reads to tell a
...
// Load caps (docs/work/dispatch.md §Dispatch), as PREDICATES rather
...
- ANY LIST OF OTHER LIVE LANES IN THIS BRIEF WENT STALE THE MOMENT IT WAS WRITTEN.
  ...
  cannot read is CANNOT TELL, not clear: take that to the worker. #4473 was
...
- A CONTROL THAT RE-QUERIES INSTEAD OF RE-USING PROVES NOTHING ABOUT YOUR GUARD.
  ...
  aside from the WORKER that was wrong, without running the one command
...
- SHUT DOWN ANY DEV SERVER BEFORE YOU REPORT. If you ran \`npm run dev\` (or
  ...
  free: the worker's worktree cleanup refuses to delete a directory with
...
- $SCRATCH may be UNSET in your shell. On THIS host it is ${STATE_DIR} — the same
  directory this script and scripts/work-checkin.sh resolve through
  scripts/work/host.mjs. export SCRATCH=${STATE_DIR} in every shell rather
...
- JOB LOGS ARE THE SAME STORY, and this line used to claim otherwise. \`GET
  ...
  2026-08-28, once by an agent and once by the worker. A tail large enough to
...
- Gates: run bash scripts/work/agent-gates.sh from the worktree root — it
...
- E2E SPLITS IN TWO, AND ONLY ONE HALF RUNS ON THIS BOX.
  ...
  * EVERY OTHER SPEC — the blast radius, the geometry-asserting sweep, the specs that
  ...
  Do NOT run the full suite locally — the worker owns full-suite runs.
...
- A PR IS AN INSTRUMENT, AND FULL CI IS A SERIAL LANDING RESOURCE. Branch pushes
  ...
  worker named this branch the NEXT LANDING CANDIDATE, push the branch but
...
- NEVER \`git stash\` — NOT \`push\`, NOT \`pop\`, NOT \`-u\`. THE STASH STACK IS SHARED
  ...
  worker's — dumps it into YOUR working copy, and CONSUMES the entry.
...
- NEVER file GitHub issues from your lane. Defects spotted in passing, premise problems,
  ...
  the worker decides what becomes an issue. Most lane findings are observations,
...
- NEVER run \`dispatch-brief.mjs done\` — retiring a dispatch is the WORKER's,
...
- Return: ${opts.candidate ? "PR number/URL" : "branch and exact head SHA"}, per-issue fix summary, VERBATIM gate results (say plainly if
  ...
  a question nobody answers; the worker turns this list into \`needs-human\`
...
// The brief is generated from whatever the worker last knew, and that
...
function cmdNew(argv) {
  ...
  // the worker's review — and that one is where a session actually jams,
  ...
  if (recent.length) {
    ...
    console.error(
      "      Not a refusal — a P0 preempts. Otherwise consider waiting: the cap that\n" +
        "      binds first is REVIEW depth, not agent count (docs/work/dispatch.md).\n"
    );
  }
  ...
  if (!rostered) {
    console.error(
      `[ledger] WARNING: could not write ${rosterPath} — work-checkin.sh will not know this agent is live.`
    );
  }
  ...
  console.error(
    `\n[ledger] recorded in ${ledgerPath} — port base ${portBase}` +
      `\n[ledger] WORKER ONLY, after the PR merges — not part of the brief above:` +
...
// that exact file in the meantime. It was stopped by ASKING the worker,
...
function cmdClaims(argv) {
  ...
  if (unknown.length) {
    ...
    console.log("  Ask the worker before touching it.");
  }
  if (verdict === "clear") {
    console.log(
...
// up, it deletes the worker's only record that the PR exists. On
...
to close the ledger entry and leave the tree alone.`
  ...
  // worker was concerned, and sat unmerged for an hour until a disk sweep
...
// closes that hole after the fact; the rule (docs/work.md, pipeline
...
function cmdAdopt(argv) {
  ...
  // `git worktree list` includes the main checkout, and the worker's own
  // branch lives there — adopting it would roster the worker as an agent.
  //
  ...
  // adopts the worker. Measured, not theorised: running this from a review
