...
// `recommend-adopt` means the worker reviews and merges it like any
...
//
// Usage:
//   node scripts/work/dependabot-eval-brief.mjs <pr-number>
//
...
- Apply the labels (REST: POST /repos/FloorLamp/allos/issues/${prNumber}/labels):
  ...
  parks with its revisit trigger in the comment; adopt means the worker
  ...
  {"assignees":["FloorLamp"]}).
- Return to the worker: the verdict, the one-line reason, and anything
  that surprised you.

Do NOT merge, close, or push to the PR yourself. On ADOPT the merge is the
WORKER's, through its normal review-and-merge flow — your comment is the
